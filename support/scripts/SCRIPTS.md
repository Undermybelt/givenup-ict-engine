...
| `support/scripts/research/ibkr_paper_roundtrip_smoke.py` | operator paper-account bridge | yes, dry-run writes a fail-closed packet and never places orders; execution requires paper port, DU/DF account, exact futures contract, and explicit paper-order confirmation | Optional IBKR paper-only roundtrip smoke for generating broker execution evidence; live ports/accounts are rejected and practical flags remain false until separate accepted-feedback conversion and same-tree closure pass |
