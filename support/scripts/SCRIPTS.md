...
| `support/scripts/done_definition_audit.py` | audit helper | yes, lightweight read-only checks by default; `--compact` emits token-friendly JSON without repo-local absolute paths, marks skipped-heavy evidence as not completion-ready, and fails closed when unsafe practical-admission wrappers remain | `python3 -m unittest support.scripts.tests.test_done_definition_audit -v` |
...
| `support/scripts/factor_claim_terminalization_audit.py` | audit helper | yes, read-only `/tmp` claim scanner; `--compact` emits token-friendly attention summaries and grouped counts | `python3 -m unittest support.scripts.tests.test_factor_claim_terminalization_audit -v` |
