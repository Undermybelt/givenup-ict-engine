...
| `support/scripts/research/futures_real_cost_rescue_audit.py` | read-only utility | yes, requires explicit CSV/JSON artifact paths | Normalizes futures rows into `rescued_for_exact_aq`, `fee_cleared_but_blocked_non_cost`, `needs_reprice_replay`, or non-rescue classes when legacy `5bps/side` stress disagrees with verified instrument-cost evidence; sample and density gates stay separate from fee survival |
| `support/scripts/research/futures_bps_false_negative_revival.py` | read-only utility | yes, requires explicit artifact files or directories | Rehears old futures Gate 1/AQ JSON packets and `autoquant_clean_*_rows.csv` / `screen_rows.csv` row packets by recomputing verified per-contract all-in cost, surfacing bps-stress false negatives while keeping zero-edge churn and unverified cost models fail-closed |
| `support/scripts/research/fixed_bps_cost_model_source_check.py` | read-only utility | yes, scans source only | Fails new source files that emit fixed-bps transaction-cost arguments, fields, or formulas as futures cost authority; legacy/readback helpers are explicitly skipped |
