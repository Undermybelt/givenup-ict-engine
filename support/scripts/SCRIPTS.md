...
| `support/scripts/research/fixed_bps_cost_model_source_check.py` | read-only utility | yes, scans source only | Fails tracked source files that emit fixed-bps transaction-cost arguments, fields, or formulas; only readback key constants may mention retired fixed-bps artifact names |
