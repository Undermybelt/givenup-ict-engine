...
""",
    ...
    def test_build_report_rejects_closure_packet_without_explicit_non_timeout_proof(self) -> None:
        with tempfile.TemporaryDirectory() as repo_tmp, tempfile.TemporaryDirectory() as claims_tmp:
            repo_root = Path(repo_tmp)
            claims_dir = Path(claims_tmp)
            run_root = repo_root / "support" / "docs" / "experiments" / "run-a"
                ...
                json.dumps(
                    {
                        ...
                        "command_results": [{"name": "all", "exit": 0}],
                    }
                ),
                encoding="utf-8",
            )
...
status=terminalized
run_root={run_root.relative_to(repo_root)}
decision=practical_closure_packet_missing_non_timeout_proof
promotion_allowed=false
trade_usable=false
""",
                encoding="utf-8",
    ...
    def test_build_report_rejects_closure_packet_requiring_funded_live_fill(self) -> None:
        with tempfile.TemporaryDirectory() as repo_tmp, tempfile.TemporaryDirectory() as claims_tmp:
            repo_root = Path(repo_tmp)
            claims_dir = Path(claims_tmp)
            run_root = repo_root / "support" / "docs" / "experiments" / "run-a"
                ...
                json.dumps(
                    {
                        ...
                        "trade_usable": True,
                        "deploy_ready": True,
                        "funded_live_fill_required": True,
                        "readiness_contract": DEPLOY_READY_READINESS_CONTRACT,
                        "provider_execution_feedback_chain": "pass",
                        "evidence_packet": "summaries/evidence-packet.json",
                    }
...
status=terminalized
run_root={run_root.relative_to(repo_root)}
decision=practical_closure_packet_funded_fill_requirement_rejected
promotion_allowed=false
trade_usable=false
""",
                encoding="utf-8",
    ...
    def test_build_report_rejects_closure_packet_with_marker_only_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as repo_tmp, tempfile.TemporaryDirectory() as claims_tmp:
            ...
            (checks_dir / "terminal_metrics.json").write_text(
                json.dumps({"chain": "provider_execution_feedback"}),
                encoding="utf-8",
            )
            (claims_dir / "terminal-practical.claim").write_text(
                f"""
...
status=terminalized
run_root={run_root.relative_to(repo_root)}
decision=practical_closure_packet_marker_only_evidence
promotion_allowed=false
trade_usable=false
""",
                encoding="utf-8",
    ...
    def test_build_report_rejects_closure_packet_without_full_lifecycle_tuple(self) -> None:
        with tempfile.TemporaryDirectory() as repo_tmp, tempfile.TemporaryDirectory() as claims_tmp:
            repo_root = Path(repo_tmp)
            claims_dir = Path(claims_tmp)
            run_root = repo_root / "support" / "docs" / "experiments" / "run-a"
...
status=terminalized
run_root={run_root.relative_to(repo_root)}
decision=practical_closure_packet_missing_lifecycle_tuple
promotion_allowed=false
trade_usable=false
""",
                encoding="utf-8",
    ...
    def test_build_report_rejects_closure_packet_without_policy_training_live_tuple(self) -> None:
        with tempfile.TemporaryDirectory() as repo_tmp, tempfile.TemporaryDirectory() as claims_tmp:
            repo_root = Path(repo_tmp)
            claims_dir = Path(claims_tmp)
            run_root = repo_root / "support" / "docs" / "experiments" / "run-a"
...
status=terminalized
run_root={run_root.relative_to(repo_root)}
decision=practical_closure_packet_missing_policy_training_lifecycle
promotion_allowed=false
trade_usable=false
""",
                encoding="utf-8",
