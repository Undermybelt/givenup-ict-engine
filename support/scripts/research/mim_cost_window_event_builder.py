...
try:
    ...
    from .mtf_trend_resonance import build_mtf_trend_resonance
...
def build_event_rows(
    ...
    *,
    ...
    product: str,
    branch_path: str,
    session_start_minute_utc: int | None = None,
    session_end_minute_utc: int | None = None,
    context_csvs: dict[str, Path] | None = None,
) -> list[dict[str, object]]:
    bars = _filter_session(
        read_bars(Path(input_csv)),
    ...
    for session_date in sorted(sessions):
        ...
        if len(session_bars) < 62:
            continue
        features = mim_cost_window_features(session_bars)
        side = 1 if features.eligible_long else 0
        label = triple_barrier_label(session_bars, EVENT_ENTRY_INDEX) if side == 1 else 0
        row: dict[str, object] = {
            "event_date": session_date,
            "event_ts": session_bars[EVENT_ENTRY_INDEX].ts.isoformat(),
            "symbol": symbol,
            "provider": provider,
            "market": market,
            "product": product,
            ...
            "mtf_trend_resonance": build_mtf_trend_resonance(
                context_csvs or {},
                event_ts=session_bars[EVENT_ENTRY_INDEX].ts.isoformat(),
                side=side,
            ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--session-start-minute-utc", type=int)
    parser.add_argument("--session-end-minute-utc", type=int)
...
def _filter_session(
    bars: Iterable[Bar],
    *,
    session_start_minute_utc: int | None,
    session_end_minute_utc: int | None,
) -> list[Bar]:
    ordered = list(bars)
    if session_start_minute_utc is None and session_end_minute_utc is None:
        return ordered
    filtered: list[Bar] = []
    for bar in ordered:
        minute = bar.ts.hour * 60 + bar.ts.minute
        if session_start_minute_utc is not None and minute < session_start_minute_utc:
            continue
        if session_end_minute_utc is not None and minute >= session_end_minute_utc:
            continue
        filtered.append(bar)
    return filtered
