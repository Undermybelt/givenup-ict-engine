...
| active | Source repo macOS CI cargo-cache repair | Owner: Codex current turn, claimed 2026-05-14 after source Actions run `25838723352` failed only macOS `Format`. Root cause evidence: `cargo fmt --check` invoked `rustup-init fmt` after `Swatinem/rust-cache@v2` restored `/Users/runner/.cargo/bin` from key `v0-rust-rust-Darwin-arm64-9d946206-6388d80f`; Ubuntu passed and release mirror passed on an older macOS runner image. Plan: stop caching cargo bin shims, bump the cache prefix, request rustfmt/clippy components explicitly, add a toolchain sanity step, then rerun CI before any release. |
