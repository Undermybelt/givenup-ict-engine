...
| blocked | 2026-05-22 three-part completion audit | Current-tree Done Definition heavy gate now passes at `/tmp/ict-engine-done-definition-audit-20260522-current-heavy.json`, and smoke path-ranker audit gate was committed as `6bcc66bb`. Release completion is still not proven: source `HEAD` is `6bcc66bb`, source `origin/main` is `79d9579e`, release mirror `main` is `ab6b1b55`, `Cargo.toml` still reports `0.1.3`, `support/docs/audits/release-signoff.md` is not present in this checkout path, and this worktree has broad dirty state. Do not publish from this checkout; rebuild a clean export for an explicit new tag and rerun release gates first. |
