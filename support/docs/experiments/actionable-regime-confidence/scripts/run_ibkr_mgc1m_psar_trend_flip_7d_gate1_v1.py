...
ROOT = BASE / "runs" / f"{STAMP}-codex-ibkr-mgc1m-psar-trend-flip-7d-gate1-v1"
...
AQ_SYMBOL = "IBKR_MGC1M_PSAR_TREND_FLIP_7D_GATE1_V1"
FACTOR_ID = "ibkr_mgc1m_psar_trend_flip_7d_gate1_v1"
BRANCH_PATH = "TrendExpansion -> PsarTrendFlip -> ibkr_mgc1m_psar_trend_flip_7d_gate1_v1"
...
VARIANTS = [
    Variant("psar_dense", 0.020, 0.18, 0.45, -0.04, 0.90, 0.0022, -0.0058, 0.0008, 0.0022),
    Variant("psar_balanced", 0.020, 0.20, 0.60, -0.01, 0.70, 0.0030, -0.0068, 0.0011, 0.0030),
    Variant("psar_quality", 0.030, 0.22, 0.75, 0.02, 0.55, 0.0042, -0.0080, 0.0014, 0.0042),
    Variant("psar_reflip", 0.015, 0.16, 0.50, -0.02, 0.80, 0.0026, -0.0062, 0.0010, 0.0026),
]
...
def class_name(variant: Variant) -> str:
    ...
    return f"IbkrMgcPsarTrendFlip{safe}1MinV1"
...
class {name}(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Vectorized approximation of Parabolic SAR using rolling extrema and acceleration bands.
        swing_low = low.shift(1).rolling(24).min()
        swing_high = high.shift(1).rolling(24).max()
        trend_bias = dataframe["ema21"] >= dataframe["ema55"]
        dataframe["psar_proxy"] = swing_low + (swing_high - swing_low) * {variant.accel / variant.accel_max:.8f}
        dataframe["psar_flip_up"] = (close > dataframe["psar_proxy"]) & (close.shift(1) <= dataframe["psar_proxy"].shift(1))
        dataframe["psar_hold_up"] = close > dataframe["psar_proxy"]
        ...
        dataframe["trend_bias"] = trend_bias
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        signal = (
            ...
            & dataframe["trend_bias"]
            & (dataframe["close"] > dataframe["ema144"] - dataframe["atr"] * 0.20)
            & (dataframe["psar_flip_up"] | (dataframe["psar_hold_up"] & (dataframe["pullback_atr"] <= {variant.pullback_atr_max})))
            & (dataframe["rvol"] >= {variant.rvol_min})
            & (dataframe["ema_slope_atr"].fillna(0) >= {variant.slope_min})
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        flip_down = dataframe["close"] < dataframe["psar_proxy"].shift(1)
        ema_lost = dataframe["close"] < dataframe["ema21"] - dataframe["atr"] * 0.35
        dataframe.loc[dataframe["force_exit_window"] | flip_down | ema_lost, "exit_long"] = 1
        return dataframe
'''
...
def write_materials(data_path: Path) -> list[Path]:
    ...
    for variant in VARIANTS:
        ...
        material = ROOT / "agent-material" / f"ibkr_mgc_psar_trend_flip_{variant.name}_1m_7d_v1.material.json"
        payload = {
            "package_id": f"ibkr-mgc-psar-trend-flip-{variant.name.replace('_', '-')}-1m-7d-v1",
            "title": f"IBKR MGC PSAR trend flip {variant.name} 1m 7D",
            "symbol": "MGC",
            "timeframe": "1m",
            ...
            "strategy_brief": "Classic Parabolic SAR / PSAR trend-flip family on MGC 1m real IBKR futures data; diversity lane distinct from VWAP, RSI, Donchian, Keltner, ADX, SuperTrend, and market-profile families.",
            ...
            "consumer_evidence_profile": {
                ...
                "source_backed_family": "Parabolic SAR / PSAR trend flip",
                "asset_class": "futures",
                "sec_type": "FUT",
                "exchange": "COMEX",
                "multiplier": "10",
                "last_trade_date": "202606",
                "gate_id": "Gate1IbkrMgcPsarTrendFlip1m7d",
                "promotion_allowed": False,
                "trade_usable": False,
                "update_goal": False,
            },
            "notes": ["public_family=parabolic_sar", "local_cache_replay=false_source_provider_root_reused", "downstream_forbidden_until_cost_density_survives"],
        }
...
def main() -> int:
    ...
    decision = "gate1_ibkr_mgc1m_psar_trend_flip_downstream_allowed" if downstream else "drop_or_block_gate1_practical"
    metrics = {
    ...
    lines = ["# Terminal Decision Summary", "", f"Decision: `{decision}`", "", "Exact MGC 1m PSAR trend-flip rows:", "", "| label | trades | win_rate | raw | 1bps | 2bps | 5bps |", "|---|---:|---:|---:|---:|---:|---:|"]
