...
SCRIPT = Path(__file__).resolve().with_name("run_ibkr_mgc1m_mama_fama_trend_turn_7d_gate1_v1.py")


def load_wrapper():
    spec = importlib.util.spec_from_file_location("ibkr_mgc_mama_fama_gate1", SCRIPT)
...
class IbkrMgcMamaFamaGate1IdentityTest(unittest.TestCase):
    def test_branch_path_is_regime_rooted_without_provider_labels(self) -> None:
        wrapper = load_wrapper()

        self.assertEqual(
            wrapper.base.BRANCH_PATH,
            "CycleTrendTurn -> MamaFamaTrendTurn -> ibkr_mgc1m_mama_fama_trend_turn_7d_gate1_v1",
        )
    ...
    def test_downstream_requires_hard_5bps_survivor_not_2bps_only(self) -> None:
        ...
        self.assertTrue(wrapper.base.hard_gate_downstream_allowed(True, ["MGC/mama_dense/1m"]))
        self.assertFalse(wrapper.base.hard_gate_downstream_allowed(False, ["MGC/mama_dense/1m"]))
