...
ROOT = BASE / "runs" / f"{STAMP}-codex-ibkr-mgc1m-adx-atr-short-fade-7d-gate1-v1"
...
AQ_SYMBOL = "IBKR_MGC1M_ADX_ATR_SHORT_FADE_7D_GATE1_V1"
FACTOR_ID = "ibkr_mgc1m_adx_atr_short_fade_7d_gate1_v1"
BRANCH_PATH = "TrendReversal -> AdxAtrShortFade -> ibkr_mgc1m_adx_atr_short_fade_7d_gate1_v1"
...
VARIANTS = [
    Variant("short_dense", 18, 0.50, 0.00020, 0.0100, 15.0, 2.0, 0.10, 0.55, 0.0028, -0.0065, 0.0010, 0.0028),
    Variant("short_balanced", 24, 0.65, 0.00025, 0.0115, 18.0, 4.0, 0.16, 0.45, 0.0040, -0.0085, 0.0014, 0.0040),
    Variant("short_quality", 36, 0.85, 0.00030, 0.0130, 21.0, 6.0, 0.22, 0.38, 0.0055, -0.0110, 0.0018, 0.0056),
    Variant("fade_fast", 14, 0.45, 0.00018, 0.0090, 13.0, 1.0, 0.08, 0.65, 0.0022, -0.0055, 0.0008, 0.0023),
]
...
def class_name(variant: Variant) -> str:
    ...
    return f"IbkrMgcAdxAtrShortFade{safe}1MinV1"
...
class {name}(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["impulse_low"] = dataframe["low"].shift(1).rolling({variant.impulse_lookback}).min()
        dataframe["pullback_high"] = dataframe["high"].shift(1).rolling(9).max()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["enter_tag"] = ""
        trend_ok = (dataframe["ema20"] < dataframe["ema50"] + dataframe["atr14"] * 0.12) & (dataframe["close"] < dataframe["ema20"] + dataframe["atr14"] * 0.20)
        directional_ok = (dataframe["adx14"] >= {variant.adx_min}) & ((-dataframe["di_margin"]) >= {variant.di_margin_min})
        impulse_ok = dataframe["close"] <= dataframe["impulse_low"] + dataframe["atr14"] * {variant.breakout_atr}
        pullback_ok = dataframe["close"] <= dataframe["pullback_high"] - dataframe["atr14"] * {variant.pullback_atr}
        signal = (
            ...
            & (dataframe["close_position"].fillna(1) <= 0.55)
            & trend_ok
        )
        dataframe.loc[signal, ["enter_short", "enter_tag"]] = (1, "{tag}")
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["exit_short"] = 0
        fail = (dataframe["di_margin"] > 1.0) | (dataframe["close"] > dataframe["ema20"] + dataframe["atr14"] * 0.60)
        dataframe.loc[dataframe["force_exit_window"] | fail, "exit_short"] = 1
        return dataframe
'''
...
def write_materials(data_path: Path) -> list[Path]:
    ...
    for variant in VARIANTS:
        ...
        material = ROOT / "agent-material" / f"ibkr_mgc_adx_atr_short_fade_{variant.name}_1m_7d_v1.material.json"
        payload = {
            "package_id": f"ibkr-mgc-adx-atr-short-fade-{variant.name.replace('_', '-')}-1m-7d-v1",
            "title": f"IBKR MGC ADX/ATR short fade {variant.name} 1m 7D",
            "symbol": "MGC",
            "timeframe": "1m",
            ...
            "strategy_brief": "Source-backed ADX/ATR bearish short fade on MGC 1m real IBKR futures data; distinct from VWAP/reclaim and micro-trend families.",
            ...
            "consumer_evidence_profile": {
                ...
                "root_symbol": "MGC",
                "root_timeframe": "1m",
                "main_regime": "TrendReversal",
                "sub_regime": "AdxAtrShortFade",
                ...
                "source_backed_family": "ADX/ATR short fade",
                "asset_class": "futures",
                "sec_type": "FUT",
                "exchange": "COMEX",
                "multiplier": "10",
                "last_trade_date": "202606",
                "gate_id": "Gate1IbkrMgcAdxAtrShortFade1m7d",
                "promotion_allowed": False,
                "trade_usable": False,
                "update_goal": False,
            },
            "notes": ["source_backed=adx_atr_short", "local_cache_replay=false_source_provider_root_reused", "downstream_forbidden_until_cost_density_survives"],
        }
...
def main() -> int:
    ...
    decision = "gate1_ibkr_mgc1m_adx_atr_short_downstream_allowed" if downstream else "drop_or_block_gate1_practical"
    metrics = {
    ...
    lines = ["# Terminal Decision Summary", "", f"Decision: `{decision}`", "", "Exact MGC 1m ADX/ATR short fade rows:", "", "| label | trades | win_rate | raw | 1bps | 2bps | 5bps |", "|---|---:|---:|---:|---:|---:|---:|"]
