...
ROOT = BASE / "runs" / f"{STAMP}-codex-ibkr-mgc1m-percentband-mfi-reclaim-7d-gate1-v1"
...
AQ_SYMBOL = "IBKR_MGC1M_PERCENTBAND_MFI_RECLAIM_7D_GATE1_V1"
FACTOR_ID = "ibkr_mgc1m_percentband_mfi_reclaim_7d_gate1_v1"
BRANCH_PATH = "RangeReversion -> PercentBandMfiReclaim -> PercentBandMfiReclaim -> ibkr_mgc1m_percentband_mfi_reclaim_7d_gate1_v1"
...
VARIANTS = [
    Variant("pbmfi_dense", 0.28, 48.0, -0.10, 0.35, 0.08, 0.0022, -0.0052, 0.0008, 0.0022),
    Variant("pbmfi_balanced", 0.18, 42.0, -0.02, 0.45, 0.14, 0.0032, -0.0068, 0.0011, 0.0032),
    Variant("pbmfi_quality", 0.10, 36.0, 0.06, 0.55, 0.20, 0.0045, -0.0088, 0.0015, 0.0045),
    Variant("pbmfi_flush", 0.06, 30.0, 0.12, 0.40, 0.28, 0.0052, -0.0100, 0.0018, 0.0052),
]
...
def class_name(variant: Variant) -> str:
    ...
    return f"IbkrMgcPercentBandMfiReclaim{safe}1MinV1"
...
class {name}(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["mid20"] = close.rolling(20).mean()
        dataframe["std20"] = close.rolling(20).std()
        dataframe["upper20"] = dataframe["mid20"] + dataframe["std20"] * 2.0
        dataframe["lower20"] = dataframe["mid20"] - dataframe["std20"] * 2.0
        band_width = (dataframe["upper20"] - dataframe["lower20"]).replace(0, 1e-9)
        dataframe["percent_b"] = (close - dataframe["lower20"]) / band_width
        ...
        pos_flow = money_flow.where(typical > typical.shift(), 0.0).rolling(14).sum()
        neg_flow = money_flow.where(typical < typical.shift(), 0.0).rolling(14).sum().replace(0, 1e-9)
        dataframe["mfi14"] = 100.0 - (100.0 / (1.0 + pos_flow / neg_flow))
        ...
        dataframe["reclaim_atr"] = (close - dataframe["lower20"]) / dataframe["atr14"].replace(0, 1)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["enter_tag"] = ""
        regime_guard = dataframe["close"] > dataframe["ema89"] - dataframe["atr14"] * 1.55
        lower_band_washout = (dataframe["percent_b"] <= {variant.percent_b_max}) & (dataframe["mfi14"] <= {variant.mfi_max})
        reclaim = dataframe["reclaim_atr"] >= {variant.reclaim_atr}
        shape = (dataframe["lower_wick_atr"] >= {variant.wick_min}) | (dataframe["close"] > dataframe["prior_low"])
        signal = dataframe["entry_window"] & regime_guard & lower_band_washout & reclaim & shape & (dataframe["rvol"] >= {variant.rvol_min})
        dataframe.loc[signal, ["enter_long", "enter_tag"]] = (1, "{tag}")
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["exit_long"] = 0
        rebound = (dataframe["percent_b"] > 0.72) | (dataframe["mfi14"] > 72)
        failed = dataframe["close"] < dataframe["lower20"] - dataframe["atr14"] * 0.50
...
def write_materials(data_path: Path) -> list[Path]:
    ...
    for variant in VARIANTS:
        ...
        material = ROOT / "agent-material" / f"ibkr_mgc_percentband_mfi_reclaim_{variant.name}_1m_7d_v1.material.json"
        payload = {
            "package_id": f"ibkr-mgc-percentband-mfi-reclaim-{variant.name.replace('_', '-')}-1m-7d-v1",
            "title": f"IBKR MGC PercentBand MFI reclaim {variant.name} 1m 7D",
            "symbol": "MGC",
            "timeframe": "1m",
            ...
            "strategy_brief": "Percent-B lower-band reclaim plus Money Flow Index washout on retained real IBKR MGC 1m futures data; distinct from Williams/MFI and RSI/VWAP families.",
            ...
            "consumer_evidence_profile": {
                ...
                "sub_regime": "PercentBandMfiReclaim",
                "sub_sub_regime_or_profit_factor": "PercentBandMfiReclaim",
                ...
                "source_backed_family": "Percent-B plus MFI lower-band reclaim",
                "asset_class": "futures",
                "sec_type": "FUT",
                "exchange": "COMEX",
                "multiplier": "10",
                "last_trade_date": "202606",
                "gate_id": "Gate1IbkrMgcPercentBandMfiReclaim1m7d",
                "promotion_allowed": False,
                "trade_usable": False,
                "update_goal": False,
            },
            "notes": ["source_backed=percent_b_mfi", "local_cache_replay=false_source_provider_root_reused", "downstream_forbidden_until_cost_density_survives"],
        }
...
def main() -> int:
    ...
    claim = Path(f"/tmp/ict-engine-agent-claims/board-b-factor-refinement/{STAMP}-codex-ibkr-mgc1m-percentband-mfi-reclaim-7d-gate1-v1.claim")
    ...
    decision = "gate1_ibkr_mgc1m_percentband_mfi_reclaim_downstream_allowed" if downstream else "drop_or_block_gate1_practical"
    metrics = {
    ...
    lines = ["# Terminal Decision Summary", "", f"Decision: `{decision}`", "", "Exact MGC 1m Percent-B / MFI reclaim rows:", "", "| label | trades | win_rate | raw | 1bps | 2bps | 5bps |", "|---|---:|---:|---:|---:|---:|---:|"]
