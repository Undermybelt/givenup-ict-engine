...
ROOT = BASE / "runs" / f"{STAMP}-codex-ibkr-mgc1m-supertrend-atr-trail-7d-gate1-v1"
...
AQ_SYMBOL = "IBKR_MGC1M_SUPERTREND_ATR_TRAIL_7D_GATE1_V1"
FACTOR_ID = "ibkr_mgc1m_supertrend_atr_trail_7d_gate1_v1"
BRANCH_PATH = "TrendExpansion -> SuperTrendAtrTrail -> ibkr_mgc1m_supertrend_atr_trail_7d_gate1_v1"
...
VARIANTS = [
    Variant("st_dense", 10, 1.7, 0.45, -0.02, 0.0024, -0.0058, 0.0009, 0.0024),
    Variant("st_balanced", 10, 2.1, 0.60, 0.00, 0.0034, -0.0070, 0.0012, 0.0034),
    Variant("st_quality", 14, 2.5, 0.75, 0.03, 0.0048, -0.0090, 0.0016, 0.0048),
    Variant("st_pullback", 14, 2.0, 0.50, -0.04, 0.0030, -0.0065, 0.0011, 0.0030),
]
...
def class_name(variant: Variant) -> str:
    ...
    return f"IbkrMgcSupertrendAtrTrail{safe}1MinV1"
...
class {name}(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["atr"] = tr.rolling({variant.atr_period}).mean()
        dataframe["ema21"] = close.ewm(span=21, adjust=False).mean()
        dataframe["ema89"] = close.ewm(span=89, adjust=False).mean()
        hl2 = (high + low) / 2.0
        dataframe["st_upper"] = hl2 + dataframe["atr"] * {variant.st_mult}
        dataframe["st_lower"] = hl2 - dataframe["atr"] * {variant.st_mult}
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["enter_tag"] = ""
        trend = (dataframe["close"] > dataframe["ema89"] - dataframe["atr"] * 0.15) & (dataframe["ema21"] >= dataframe["ema89"] - dataframe["atr"] * 0.05)
        reclaim = (dataframe["close"] > dataframe["st_upper"].shift(1)) | (dataframe["close"] > dataframe["prior_high"])
        pullback_hold = dataframe["close"] > dataframe["st_lower"].shift(1)
        signal = (
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        trend_lost = dataframe["close"] < dataframe["st_lower"].shift(1)
        ema_lost = dataframe["close"] < dataframe["ema21"] - dataframe["atr"] * 0.40
        dataframe.loc[dataframe["force_exit_window"] | trend_lost | ema_lost, "exit_long"] = 1
        return dataframe
'''
...
def write_materials(data_path: Path) -> list[Path]:
    ...
    for variant in VARIANTS:
        ...
        material = ROOT / "agent-material" / f"ibkr_mgc_supertrend_atr_trail_{variant.name}_1m_7d_v1.material.json"
        payload = {
            "package_id": f"ibkr-mgc-supertrend-atr-trail-{variant.name.replace('_', '-')}-1m-7d-v1",
            "title": f"IBKR MGC SuperTrend ATR trail {variant.name} 1m 7D",
            "symbol": "MGC",
            "timeframe": "1m",
            ...
            "strategy_brief": "Public SuperTrend ATR trailing breakout on MGC 1m real IBKR futures data; diversity lane distinct from Donchian, ADX, RSI/VWAP, and opening VWAP families.",
            ...
            "consumer_evidence_profile": {
                ...
                "source_backed_family": "SuperTrend/ATR trail breakout",
                "asset_class": "futures",
                "sec_type": "FUT",
                "exchange": "COMEX",
                "multiplier": "10",
                "last_trade_date": "202606",
                "gate_id": "Gate1IbkrMgcSuperTrendAtrTrail1m7d",
                "promotion_allowed": False,
                "trade_usable": False,
                "update_goal": False,
            },
            "notes": ["source_backed=supertrend_atr", "local_cache_replay=false_source_provider_root_reused", "downstream_forbidden_until_cost_density_survives"],
        }
...
def main() -> int:
    ...
    decision = "gate1_ibkr_mgc1m_supertrend_atr_trail_downstream_allowed" if downstream else "drop_or_block_gate1_practical"
    metrics = {
    ...
    lines = ["# Terminal Decision Summary", "", f"Decision: `{decision}`", "", "Exact MGC 1m SuperTrend/ATR rows:", "", "| label | trades | win_rate | raw | 1bps | 2bps | 5bps |", "|---|---:|---:|---:|---:|---:|---:|"]
