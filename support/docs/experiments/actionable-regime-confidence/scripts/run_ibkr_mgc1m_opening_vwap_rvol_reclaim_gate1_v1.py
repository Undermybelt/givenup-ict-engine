...
ROOT = BASE / "runs" / f"{STAMP}-codex-ibkr-mgc1m-opening-vwap-rvol-reclaim-gate1-v1"
...
AQ_SYMBOL = "IBKR_MGC1M_OPENING_VWAP_RVOL_RECLAIM_GATE1_V1"
FACTOR_ID = "ibkr_mgc1m_opening_vwap_rvol_reclaim_gate1_v1"
BRANCH_TEMPLATE = "RangeReversion -> OpeningVwapRvolReclaim -> ibkr_mgc1m_opening_vwap_rvol_reclaim_gate1_v1"
...
def main() -> int:
    ...
    for index, contract in enumerate(CONTRACTS, start=1):
        ...
        for variant in VARIANTS:
            ...
            branch = BRANCH_TEMPLATE.format(product=contract.product, symbol=contract.symbol)
    ...
    branch_ok = bool(rank_rows) and branch_paths == [BRANCH_TEMPLATE]
    ...
    lines += ["", f"- `branch_fields_preserved={branch_ok}`", f"- `exact_1m_survivors_2bps={survivors_2}`", f"- `downstream_allowed={downstream}`", ""]
