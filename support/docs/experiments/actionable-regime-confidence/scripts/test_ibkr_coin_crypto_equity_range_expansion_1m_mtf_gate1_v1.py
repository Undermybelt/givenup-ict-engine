...
SCRIPT = Path(__file__).resolve().with_name("run_ibkr_coin_crypto_equity_range_expansion_1m_mtf_gate1_v1.py")


def load_runner():
    spec = importlib.util.spec_from_file_location("ibkr_coin_crypto_equity_range_expansion_gate1", SCRIPT)
...
class IbkrCoinCryptoEquityRangeExpansionGate1Test(unittest.TestCase):
    ...
    def test_rank_rows_fail_closed_when_coin_equity_cost_model_is_unverified(self) -> None:
        runner = load_runner()

        rows = runner.instrument_cost_rows(
            [
                {
                    "package_id": "ibkr-coin-crypto-equity-range-expansion-1m-30d-v1",
                    "unit_label": "toy",
                    "timeframe": "1m",
                    "trade_count": 45,
                    "total_profit_pct": 19.0,
                    "win_rate_pct": 58.0,
                    "sharpe": 1.4,
                }
            ],
        ...
        row = rows[0]
        self.assertEqual(row["raw_total_profit_pct"], 19.0)
    ...
    def test_material_payload_uses_instrument_cost_verification_not_fixed_bps_priority(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir, mock.patch.object(runner, "ROOT", Path(tmpdir)):
            normalized = Path(tmpdir) / "data" / "ibkr_coin_1m_30d.csv"
    ...
    def test_terminal_summary_and_claim_text_do_not_emit_fixed_bps_cost_authority(self) -> None:
        runner = load_runner()
        metrics = {
        ...
        with tempfile.TemporaryDirectory() as tmpdir, mock.patch.object(runner, "ROOT", Path(tmpdir)):
            ...
            summary_text = (Path(tmpdir) / "summaries" / "terminal_decision_summary.md").read_text(encoding="utf-8")
            claim_text = runner.terminal_claim_text(metrics["decision"], [], False)

        self.assertIn("Instrument Cost Verification Table", summary_text)
        self.assert_no_fixed_cost_bps_text(summary_text)
        self.assert_no_fixed_cost_bps_text(claim_text)
