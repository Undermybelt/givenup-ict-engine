...
SCRIPT = Path(__file__).resolve().with_name("run_ibkr_cvx_psar_adx_trend_aq_gate1_v1.py")


def load_runner():
    spec = importlib.util.spec_from_file_location("ibkr_cvx_psar_adx_trend_aq_gate1_v1", SCRIPT)
...
class IbkrCvxPsarAdxTrendAqGate1Test(unittest.TestCase):
    ...
    def test_rank_rows_fail_closed_when_equity_cost_model_is_unverified(self) -> None:
        runner = load_runner()

        rows = runner.instrument_cost_rows(
            [
                {
                    "package_id": "ibkr-cvx-psar-adx-trend-1m-30d-v1",
                    "unit_label": "toy",
                    "timeframe": "1m",
                    "trade_count": 20,
        ...
        row = rows[0]
        self.assertEqual(row["raw_total_profit_pct"], 8.0)
    ...
    def test_material_payload_uses_cost_verification_not_fixed_bps_priority(self) -> None:
        runner = load_runner()

        with tempfile.TemporaryDirectory() as tmpdir, mock.patch.object(runner, "ROOT", Path(tmpdir)):
            normalized = Path(tmpdir) / "data" / "ibkr_cvx_1m_30d.csv"
        ...
        rows = [
            {
                "timeframe": "1m",
                "trade_count": 20,
                "trades_per_day": 0.667,
                "raw_total_profit_pct": 8.0,
