...
def main() -> int:
    ...
    write_json(REPORT_DIR / "112315_same_root_six_provider_catboost_feature_support_v1.json", summary)

    report = REPORT_DIR / "112315_same_root_six_provider_catboost_feature_support_v1.md"
    assertions = CHECK_DIR / "112315_same_root_six_provider_catboost_feature_support_v1_assertions.out"
    report.write_text(
        "\n".join(
            [
                "# 112315 Same-Root Six-Provider CatBoost Feature Support v1",
                "",
                f"Run id: `{RUN_ID}`",
                f"Symbol: `{SYMBOL}`",
                "",
                ...
                "",
                "## Decision",
                "- Gate result: `112315_same_root_six_provider_catboost_feature_support=catboost_runtime_repaired_but_execution_fail_closed`.",
                "- `promotion_allowed=false`.",
                "- `trade_usable=false`.",
                "- `update_goal=false`.",
            ]
