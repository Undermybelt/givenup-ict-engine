...
class TomacNQ_VM_Sideways18_AM12_15(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["low_window"] = dataframe["low"].rolling(18).min().shift(1)
        dataframe["hour_utc"] = dataframe["date"].dt.hour
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        am_killzone = (dataframe["hour_utc"] >= 12) & (dataframe["hour_utc"] <= 15)
