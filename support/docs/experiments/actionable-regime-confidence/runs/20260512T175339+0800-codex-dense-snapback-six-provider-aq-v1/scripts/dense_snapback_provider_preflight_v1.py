...
RUN_ID = "20260512T175339+0800-codex-dense-snapback-six-provider-aq-v1"
...
BRANCH_PATHS = [
    "RangeReversion -> NormalVolatility -> Rsi2LowerTailSnapback -> rsi2_lower_tail_snapback_v1",
    "RangeReversion -> NormalVolatility -> BollingerLowerBandSnapback -> bb_lower_band_snapback_v1",
    "RangeReversion -> NormalVolatility -> KeltnerLowerBandSnapback -> keltner_lower_band_snapback_v1",
    "TrendTransition -> NormalVolatility -> Ema20ReclaimContinuation -> ema20_reclaim_continuation_v1",
]

STRATEGY_TEXT = '''from freqtrade.strategy import IStrategy
...
def material(package_id: str, title: str, symbol: str, data_path: Path, provider_note: str) -> dict[str, Any]:
    return {
        ...
        "strategy_source_path": str(RUN_ROOT / "agent-material/ProviderDenseSnapbackV1.py"),
        "strategy_class_name": "ProviderDenseSnapbackV1",
        "strategy_brief": "Board B dense RSI/Bollinger/Keltner snapback branch packet.",
        "evaluation_priority": [
        ...
        "consumer_evidence_profile": {
            "main_regime": "RangeReversion_or_TrendTransition",
...
def main() -> int:
    ...
    write_text(RUN_ROOT / "agent-material/ProviderDenseSnapbackV1.py", STRATEGY_TEXT)
    ...
    for label, args, raw_path, norm_path, provider, note, symbol in fetch_jobs:
        ...
        if exit_code == 0 and raw_path.exists():
            ...
            package_id = f"dense-snapback-{provider.lower().replace('/', '-').replace(' ', '-')}-1h-v1"
            material_path = RUN_ROOT / "agent-material" / f"{package_id}.material.json"
            write_json(material_path, material(package_id, f"Dense snapback - {provider} {symbol} 1h", symbol, norm_path, note))
    ...
    tvr_exit = run_or_reuse(
        ...
        [
            ...
            "fetch",
            "--market",
            "board-b-175339-dense-snapback-tvr-btc-usd-1h",
            "--interval",
            "1h",
            "--role",
            "crypto_reference",
        ...
        env={
            "HOME": "/tmp/ict-engine-tvr-local-stdio-175339-home",
    ...
    if tvr_exit == 0:
        ...
        package_id = "dense-snapback-tvr-btc-usd-1h-v1"
        material_path = RUN_ROOT / "agent-material" / f"{package_id}.material.json"
        write_json(material_path, material(package_id, "Dense snapback - TVR BTC-USD 1h", "BTC-USD", tvr_norm, "source_provider=TradingViewRemix/TVR local-stdio BTC-USD 1h current fetch"))
    ...
    write_json(RUN_ROOT / "summaries/dense_snapback_provider_preflight_v1.json", summary)
