#!/usr/bin/env python3
"""Completion audit after the R6 Shak uplift.
...
RUN_ID = "20260511T212053-codex-current-goal-completion-audit-v42-after-shak-uplift"
...
ARTIFACTS = {
    ...
    "r6_shak_complaint_gate": RUNS_DIR
    / "20260511T211606-codex-r6-shak-complaint-row-uplift-gate-v1/"
    "r6-shak-complaint-row-uplift-gate/r6_shak_complaint_row_uplift_gate_v1.json",
    "r6_shak_cftc_uplift": RUNS_DIR
    / "20260511T211628-codex-r6-shak-cftc-row-uplift-v1/"
    "r6-shak-cftc-row-uplift/r6_shak_cftc_row_uplift_v1.json",
    "r6_vorley_chanu_uplift": RUNS_DIR
    / "20260511T211208-codex-cftc-vorley-chanu-row-uplift-calibration-v1/"
    "r6-cftc-vorley-chanu-row-uplift-calibration/cftc_vorley_chanu_row_uplift_calibration_v1.json",
...
def main() -> int:
    ...
    checklist = [
        ...
        },
        {
            ...
            "evidence": f"current_cursor={cursor.get('last_loop_id')}; ready_roots={len(ready_roots)}/4; direct_min_wilson95_lcb={min_lcb:.6f}",
            "status": "fail_blocked",
        ...
        },
        {
            ...
            "requirement": "Validate on other markets/species with suitable confidence.",
        ...
        },
        {
            ...
            "requirement": "Validate on other cycles/timeframes with suitable confidence.",
        ...
        },
        {
            ...
            "requirement": "Direct Manipulation must pass support, Wilson95, broad-normal, and species gates.",
            ...
            "status": "fail_blocked",
            "gap": "R6 improved to 37/37 but support is still below 50/50, Wilson95 is below 0.95, controls are same-event seeds, and species coverage is incomplete.",
        },
        {
            "id": "R5_no_proxy_acceptance",
            "requirement": "Do not accept proxy signals, generated labels, no-send drafts, or same-event controls as completion.",
            "evidence": "This audit keeps VantMacro drafts, source panels without new rows, same-event controls, and OHLCV/provider proxies fail-closed.",
    ...
    decision = f"current_goal_completion_audit_v42=post_shak_live_{positive_rows}x{matched_negative_rows}_still_blocked"
    result = {
        "run_id": RUN_ID,
    ...
    json_path = OUT_DIR / "current_goal_completion_audit_v42_after_shak_uplift.json"
    checklist_csv = OUT_DIR / "current_goal_completion_audit_v42_checklist.csv"
    gates_csv = OUT_DIR / "current_goal_completion_audit_v42_gates.csv"
    roots_csv = OUT_DIR / "current_goal_completion_audit_v42_intake_roots.csv"
    report_path = OUT_DIR / "current_goal_completion_audit_v42_after_shak_uplift.md"
    assertions_path = CHECK_DIR / "current_goal_completion_audit_v42_after_shak_uplift_assertions.out"
    ...
    print(json.dumps({"decision": decision, "ready_roots": len(ready_roots), "r6_positive_rows": positive_rows, "r6_matched_negative_rows": matched_negative_rows, "r6_min_wilson95_lcb": round(min_lcb, 6), "update_goal": False, "report": str(report_path.relative_to(REPO))}, indent=2))
    return 0
