...
RUN_ID = "20260512T032157+0800-codex-board-b-nq-cost-crisis-repair-v3"
SCHEMA_VERSION = "board-b-nq-cost-crisis-repair/v3"
RECIPE_ID = "NQRootAdaptiveCostCrisisRepairV3"
...
REPAIR_VARIANTS = [
    ...
    {
        "root": "Sideways",
        "variant_id": "sideways_calm_vix_z_revert_h72",
        "branch_path": branch_path("Sideways", "RangeConsolidation", "NQCalmVixZReversion", "sideways_calm_vix_z_revert_h72"),
        "hold": 72,
        "action": "z_revert",
        "source_vix_max": 14.41,
    },
    {
        "root": "Sideways",
...
def main() -> int:
    ...
    base.OUT_DIR = RUN_ROOT / "nq-cost-crisis-repair-v3"
    base.CHECK_DIR = RUN_ROOT / "checks"
    base.COMMAND_DIR = RUN_ROOT / "command-output"
    base.STATE_SYMBOL_DIR = RUN_ROOT / "state_nq_cost_crisis_repair_v3" / SYMBOL
    base.REPORT_JSON = base.OUT_DIR / "nq_cost_crisis_repair_rc_spa_v3.json"
    base.REPORT_MD = base.OUT_DIR / "nq_cost_crisis_repair_rc_spa_v3.md"
    base.VARIANT_ROWS_CSV = base.OUT_DIR / "nq_cost_crisis_repair_variant_rows_v3.csv"
    base.SELECTED_ROWS_CSV = base.OUT_DIR / "nq_cost_crisis_repair_selected_rows_v3.csv"
    base.BRANCH_SUMMARY_CSV = base.OUT_DIR / "nq_cost_crisis_repair_branch_summary_v3.csv"
    base.INPUTS_JSON = base.OUT_DIR / "nq_cost_crisis_repair_inputs_v3.json"
    base.REAL_TRADES = base.OUT_DIR / "nq_cost_crisis_repair_real_trades_v3.jsonl"
    base.STRATEGY_LIBRARY = base.OUT_DIR / "strategy_library_nq_cost_crisis_repair_v3.json"
    base.ASSERTIONS = base.CHECK_DIR / "nq_cost_crisis_repair_v3_assertions.out"

    result = int(base.main())

    library = json.loads(base.STRATEGY_LIBRARY.read_text(encoding="utf-8"))
    library["manifest_version"] = "1.0"
    for entry in library.get("strategies", []):
        entry["file_path"] = base.rel(Path(__file__))
        entry.setdefault("metadata", {})["mutation_id"] = "b2r_nq_cost_crisis_repair_v3"
    base.STRATEGY_LIBRARY.write_text(
