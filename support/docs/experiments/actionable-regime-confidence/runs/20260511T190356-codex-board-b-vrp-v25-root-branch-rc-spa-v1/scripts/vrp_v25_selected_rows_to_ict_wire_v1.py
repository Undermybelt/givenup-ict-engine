#!/usr/bin/env python3
"""Convert selected VRP V2.5 Board B rows to ict-engine wire JSONL."""
...
SYMBOL = "VRP_PANEL_V25_190356_SELECTED"
SCHEMA_VERSION = "1.0"

HERE = Path(__file__).resolve().parent
RUN_ROOT = HERE.parent
INPUT_CSV = RUN_ROOT / "branch-rc-spa" / "vrp_v25_selected_branch_rows_v1.csv"
OUTPUT_JSONL = RUN_ROOT / "ict-engine-fail-closed" / "vrp_v25_selected_real_trades_wire_v1.jsonl"
COUNT_FILE = RUN_ROOT / "ict-engine-fail-closed" / "vrp_v25_selected_real_trades_wire_v1.count"
SAMPLE_JSON = RUN_ROOT / "ict-engine-fail-closed" / "vrp_v25_selected_real_trades_wire_v1.sample.json"
...
def record_from_row(row: dict[str, str], idx: int, total: int) -> dict[str, Any]:
    ...
    trade_id = (
        f"{RECIPE_ID}|{row['variant_id']}|{row['market']}|"
        f"{row['timeframe']}|{row['open_date']}|{idx}"
    )
    return {
        ...
        "strategy_mutation_id": f"board-b-vrp-v25-selected-{row['variant_id']}",
        ...
        "factors_used": [
            ...
            {
                "factor_name": row.get("profit_factor_leaf") or row["variant_id"],
                "category": row.get("profit_factor_family") or "options_hedging",
                "direction": factor_direction_for_pnl(pnl),
                "value": pnl,
                "confidence": 0.65,
                "weighted_score": pnl,
        ...
        "structural_feedback": {
            ...
            "notes": "diagnostic_dry_run_only_vrp_v25_rc_spa_rejected",
        },
    }
