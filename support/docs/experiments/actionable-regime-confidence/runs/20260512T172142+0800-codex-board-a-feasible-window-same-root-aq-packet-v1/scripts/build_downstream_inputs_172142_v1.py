#!/usr/bin/env python3
"""Build run-local ict-engine downstream inputs for Board A 172142."""
...
PACKET = (
    ...
    / "feasible_window_same_root_aq_packet_v1.json"
)
OUT_LIBRARY = ROOT / "provider_btc_172142_strategy_library_v1.json"
...
def strategy_file(provider: str, strategy: str) -> str:
    matches = sorted(
        (ROOT / "workspace").glob(
            f"same-root-aq-v1/auto-quant-112315-{provider}/user_data/strategies_external/{strategy}.py"
        )
    )
...
def build_library() -> None:
    ...
    for result in packet["aq_results"]:
        ...
        for strategy, metrics_payload in result.get("metrics", {}).items():
            aggregate = metric_dict(metrics_payload.get("aggregate", {}))
            mutation_id = f"172142_{provider}_{strategy}"
            strategies.append(
                {
                    "name": f"{strategy}_{provider}_172142",
                    "file_path": strategy_file(provider, strategy),
                    "metadata": {
                        "strategy": strategy,
                        "mutation_id": mutation_id,
                        "base_factor": "feasible_window_six_provider_btc_provider_aq",
                        "hypothesis": (
                            "Board A BTC 1h feasible-window six-provider AQ evidence packet; "
                            "provider-backed downstream readback only, not acceptance."
                        ),
                        "paradigm": "provider_backed_crypto_regime_probe",
                        "expected_regime": "BoardA -> BTC -> feasible_window_six_provider_aq_surface",
                        "factors_used": [
                            "provider_crypto_momentum",
                            "provider_crypto_pullback_reversion",
                            "six_provider_feasible_window_agreement",
                        ],
                        "parent": "BoardA_BTC_172142",
                        "asset_class": "crypto_cross_provider",
                        "status": "provider_backed_auto_quant_downstream_input",
                        "created": "2026-05-12T17:21:42+08:00",
                    },
                    "status": "ok",
    ...
    manifest = {
        ...
        "log_path": str(
            ROOT
            / "feasible-window-same-root-aq-packet-v1"
            / "feasible_window_same_root_aq_packet_v1.md"
        ),
