...
RUN_ROOT = (
    ...
    "20260511T032326-codex-expanded-root-gate-mtf"
)
...
LOOP_ID = "20260511T032326+0800-codex-expanded-root-gate-mtf"
...
def assign_source_backed_labels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    labels.loc[consolidation] = "SidewaysConsolidation"
    ...
    out["target_source_backed_SidewaysConsolidation"] = labels == "SidewaysConsolidation"
...
def schema(row_counts: dict[str, dict[str, int]], contexts: dict[str, int], numeric_cols: list[str], boolean_cols: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "expanded-main-regime-root-schema/v1",
        ...
        "target_definitions": {
            ...
            "SidewaysConsolidation": "target_ConsolidationRange_next true after Crisis/Bull/Bear priority",
...
def crosswalk() -> dict[str, Any]:
    return {
        "schema_version": "expanded-main-regime-crosswalk/v1",
        ...
        "crosswalk": {
            ...
            "SidewaysConsolidation": {
                ...
                "promotion_requirement": "root-level sideways/consolidation target passes 95 gate directly",
            },
            "CrisisStress": {
...
def main() -> int:
    ...
    feature_table = ROOT_DIR / "expanded_root_feature_table.csv"
    df.to_csv(feature_table, index=False)
    schema_path = ROOT_DIR / "expanded_root_schema.json"
    crosswalk_path = ROOT_DIR / "expanded_root_crosswalk.json"
    report_path = ROOT_DIR / "expanded_root_gate_report.json"
    summary_path = ROOT_DIR / "expanded_root_gate_summary.csv"
    ...
    report = {
        ...
        "objective": (
            "Rerun unchanged 95% chronological gates under the expanded active Board A root ontology "
        ...
        "decision": {
            ...
            "accepted_95_all_expanded_roots": len(missing) == 0,
            "accepted_gate": (
                "expanded_main_regime_roots_accepted_95_all"
                if not missing
                else "partial_for_expanded_main_regime_roots"
            ),
            "trade_usable": False,
            "blocker": "missing_root_classes_95=" + ",".join(missing),
            "next_action": (
                "Stop proxy-only searches for Manipulation; obtain direct order-flow/L2/order-lifecycle "
                "or event labels, and for any remaining expansion/consolidation roots use materially new parent-labeled data before rerun."
            ),
        },
    }
    ...
    (CHECKS_DIR / "expanded_root_gate_assertions.out").write_text("\n".join(assertions) + "\n", encoding="utf-8")
    (RUN_ROOT / "README.md").write_text(
        "# Expanded Main-Regime Root Gate MTF\n\n"
        "This run materializes the expanded active root ontology from the 2026-05-11 reset and reruns "
        ...
        f"- assertions: `{repo_rel(CHECKS_DIR / 'expanded_root_gate_assertions.out')}`\n",
        encoding="utf-8",
    )
    (RUN_ROOT / "completion-audit/prompt_to_artifact_completion_audit.md").write_text(
        ...
        "- Artifact produced: expanded active root gate over two market families and 15m/1h timeframes.\n"
        ...
        "- Completion judgment: not complete for the full user objective unless every expanded root except residual passes and Manipulation has direct required inputs.\n",
        encoding="utf-8",
    )
