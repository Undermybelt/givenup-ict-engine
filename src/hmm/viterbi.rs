...
impl Viterbi {
    ...
    pub fn decode(observations: &[Vec<f64>], params: &HMMParams) -> (Vec<usize>, f64) {
        ...
        for state in 0..k {
            ...
            log_delta[0][state] = params.initial_probs[state].ln() + log_emit;
        }
        ...
        for tt in 1..t {
            for state in 0..k {
                ...
                for prev_state in 0..k {
                    let val =
                        log_delta[tt - 1][prev_state] + params.transition[prev_state][state].ln();
                    if val > best {
                        best = val;
        ...
        for state in 0..k {
            if log_delta[t - 1][state] > log_likelihood {
                log_likelihood = log_delta[t - 1][state];
                path[t - 1] = state;
            }
        }
