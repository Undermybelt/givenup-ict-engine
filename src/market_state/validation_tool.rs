...
use crate::market_state::{
    MarketStateClassifier, MarketStateSnapshot, PrimaryMarketRegime, SecondaryMarketRegime,
    ConfidenceLevel,
};
...
impl MarketStateValidator {
            ...
            *secondary_dist.entry(secondary_key).or_insert(0) += 1;
            ...
            if self.config.verbose && i % (step * 100) == 0 {
                println!(
                    ...
                    i, candles.len(), snapshot.primary_regime, snapshot.secondary_regime, snapshot.overall_confidence
                );
            }
        }
    ...
    pub fn generate_report(&self, result: &ValidationResult) -> String {
        ...
        report.push_str(&format!("High Confidence Ratio: {:.2}%\n", result.high_confidence_ratio * 100.0));
        report.push_str(&format!("Tradeable Ratio: {:.2}%\n\n", result.tradeable_ratio * 100.0));
        ...
        report.push_str(&format!("  High    (≥0.75): {:>6} ({:>5.1}%)\n", dist.high, high_pct));
        report.push_str(&format!("  Medium  (≥0.55): {:>6} ({:>5.1}%)\n", dist.medium, medium_pct));
        report.push_str(&format!("  Low     (≥0.35): {:>6} ({:>5.1}%)\n", dist.low, low_pct));
        report.push_str(&format!("  VeryLow (<0.35): {:>6} ({:>5.1}%)\n", dist.very_low, very_low_pct));
        report.push_str("\n");
