...
use ict_engine::data::{
    aggregate_candles_by_minutes, candles_to_prices, load_candles, load_tomac_continuous_candles,
    realtime::{
...
#[derive(Debug, Serialize)]
struct AnalyzeMultiTimeframeSection {
...
fn build_analyze_multi_timeframe_section(
    summary: &[String],
...
fn build_expansion_factor_pipeline_report(
    ...
    let factor_engine = FactorEngine::new(registry);
...
fn raw_liquidity_context_trace(
    liquidity_label: &str,
    frame: &FrameFeatures,
) -> FactorPipelineLabelSource {
    FactorPipelineLabelSource {
...
fn raw_multi_timeframe_resonance_trace(
    policy: &ict_engine::state::PreBayesEvidencePolicy,
    pre_bayes_filter: &PreBayesEvidenceFilter,
    multi_timeframe_evidence: &ParsedMultiTimeframeEvidence,
    regime_label: &str,
...
fn multi_timeframe_entry_quality_bias(
    evidence: &ParsedMultiTimeframeEvidence,
    direction: Direction,
) -> Vec<f64> {
...
fn pre_bayes_evidence_policy() -> ict_engine::state::PreBayesEvidencePolicy {
    let min_directional_support_gap = env_f64("ICT_ENGINE_PREBAYES_MIN_SUPPORT_GAP", 0.08);
    let high_uncertainty_threshold =
        env_f64("ICT_ENGINE_PREBAYES_HIGH_UNCERTAINTY_THRESHOLD", 0.45);
    let min_multi_timeframe_alignment_score =
        env_f64("ICT_ENGINE_PREBAYES_MIN_MTF_ALIGNMENT_SCORE", 0.55);
    let min_multi_timeframe_entry_alignment_score =
        env_f64("ICT_ENGINE_PREBAYES_MIN_MTF_ENTRY_ALIGNMENT_SCORE", 0.50);
    let hard_pass_quality_threshold =
        env_f64("ICT_ENGINE_PREBAYES_HARD_PASS_QUALITY_THRESHOLD", 0.75);
    let neutralized_quality_threshold =
        env_f64("ICT_ENGINE_PREBAYES_NEUTRALIZED_QUALITY_THRESHOLD", 0.40);
    let directional_conflict_penalty =
        env_f64("ICT_ENGINE_PREBAYES_DIRECTIONAL_CONFLICT_PENALTY", 0.20);
    let mixed_alignment_penalty = env_f64("ICT_ENGINE_PREBAYES_MIXED_ALIGNMENT_PENALTY", 0.10);
    let multi_timeframe_direction_conflict_penalty =
        env_f64("ICT_ENGINE_PREBAYES_MTF_DIRECTION_CONFLICT_PENALTY", 0.18);
    let multi_timeframe_alignment_penalty =
        env_f64("ICT_ENGINE_PREBAYES_MTF_ALIGNMENT_PENALTY", 0.10);
    let multi_timeframe_entry_penalty = env_f64("ICT_ENGINE_PREBAYES_MTF_ENTRY_PENALTY", 0.08);
    let multi_timeframe_alignment_bonus = env_f64("ICT_ENGINE_PREBAYES_MTF_ALIGNMENT_BONUS", 0.05);
    let hostile_liquidity_penalty = env_f64("ICT_ENGINE_PREBAYES_HOSTILE_LIQUIDITY_PENALTY", 0.10);
    let favorable_liquidity_bonus = env_f64("ICT_ENGINE_PREBAYES_FAVORABLE_LIQUIDITY_BONUS", 0.05);
    let hostile_liquidity_forces_high_uncertainty = env_bool(
...
fn build_pre_bayes_evidence_filter(
    policy: &ict_engine::state::PreBayesEvidencePolicy,
    regime_label: &str,
    liquidity_label: &str,
    factor_diagnostics: &FactorDiagnostics,
