#!/usr/bin/env python3
"""Board B NQ root-aware B2U RC-SPA evidence builder.

This is an additive experiment artifact. It writes a temporary Freqtrade
strategy and synthetic validation data under this run root, uses local
Auto-Quant/Freqtrade, and does not modify ict-engine runtime code or the
Auto-Quant checkout.

Freqtrade validates pairs against an exchange market list before reading local
data. This probe therefore maps real local `NQ_USD-1d` bars to the exchange-
compatible label `BTC/USDT` inside the run root. Every emitted row keeps the
real source symbol (`NQ_USD`) and the adapter label separately.
"""
...
RUN_ID = "20260511T184100+0800-codex-board-b-nq-rootaware-b2u-v1"
SCHEMA_VERSION = "board-b-nq-rootaware-b2u-rc-spa/v1"
RECIPE_ID = "NQRootAwareB2UV1"
STRATEGY_CLASS = "NQRootAwareB2UV1"
REAL_SOURCE_SYMBOL = "NQ_USD"
FREQTRADE_ADAPTER_PAIR = "BTC/USDT"
FREQTRADE_ADAPTER_FILE = "BTC_USDT-1d.feather"
TIMEFRAME = "1d"
PAIRS = [FREQTRADE_ADAPTER_PAIR]
TIMERANGES = [
    ("nq_full_2011_2025", "20110101-20251231"),
]
...
AUTO_QUANT_SOURCE_DATA = AUTO_QUANT_ROOT / "user_data" / "data"
AUTO_QUANT_DATA = RUN_ROOT / "freqtrade-adapter-data"
...
def prepare_freqtrade_adapter_data() -> None:
    AUTO_QUANT_DATA.mkdir(parents=True, exist_ok=True)
    source_path = AUTO_QUANT_SOURCE_DATA / "NQ_USD-1d.feather"
    if not source_path.exists():
        raise FileNotFoundError(source_path)
    df = pd.read_feather(source_path)
    df["date"] = pd.to_datetime(df["date"], unit="ms", utc=True)
    df.to_feather(AUTO_QUANT_DATA / FREQTRADE_ADAPTER_FILE)


def apply_variant_env(variant: dict[str, Any]) -> None:
    os.environ["BOARD_B_BULL_BREAKOUT"] = str(variant["bull_breakout"])
    ...
    os.environ["BOARD_B_HOLD_DAYS"] = str(variant["hold_days"])
        ...
        attached.append(
            {
                ...
                "pair": trade.get("pair", ""),
                "freqtrade_adapter_pair": FREQTRADE_ADAPTER_PAIR,
                "source_market_symbol": REAL_SOURCE_SYMBOL,
                "source_market_timeframe": TIMEFRAME,
...
def summarize_branch(
    ...
    depth_score = min(max(n / REQUIRED_TRADES, 0.0), 1.0)
...
def main() -> int:
    ...
    prepare_freqtrade_adapter_data()
    ...
    with backtest_log.open("w", encoding="utf-8") as out, backtest_err.open("w", encoding="utf-8") as err:
        with redirect_stdout(out), redirect_stderr(err):
            for label, timerange in TIMERANGES:
                ...
                if label == TIMERANGES[-1][0]:
                    selected_trades = trades
            for variant in VARIANTS:
                results = run_backtest(TIMERANGES[-1][1], variant)
                strategy_result = extract_strategy_result(results)
                trades = extract_trades(strategy_result)
    ...
    crisis_days = int(
        daily_context[
            (daily_context["date"] >= pd.Timestamp("2011-01-01", tz="UTC"))
    ...
    decision = {
        ...
        "primary_blocker": (
            "NQRootAwareB2UV1 emits root-first branches and real Auto-Quant/Freqtrade "
            ...
            "Crisis sessions in 2011-2025), so downstream runtime promotion remains blocked."
        ),
        "next_action": (
    ...
    report = {
        ...
        "auto_quant": {
            ...
            "source_market_symbol": REAL_SOURCE_SYMBOL,
            "freqtrade_adapter_pair": FREQTRADE_ADAPTER_PAIR,
            "freqtrade_adapter_note": (
                "NQ_USD bars are copied to BTC_USDT-1d.feather inside the run root only "
                "because Freqtrade validates local data names against exchange markets."
            ),
