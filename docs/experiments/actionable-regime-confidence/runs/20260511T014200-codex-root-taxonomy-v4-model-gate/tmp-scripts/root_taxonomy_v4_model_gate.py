...
RUN_ROOT = (
    ...
    "20260511T014200-codex-root-taxonomy-v4-model-gate"
)
ROOT_DIR = RUN_ROOT / "root-v4"
...
SOURCE_SCHEMA = (
    REPO
    / "docs/experiments/actionable-regime-confidence/runs/"
    "20260511T013403-codex-web-backed-root-taxonomy-v4-refresh/schema/"
    "root_taxonomy_v4_schema.json"
)

LOOP_ID = "20260511T014200+0800-codex-root-taxonomy-v4-model-gate"
REQUIRED_ROOTS = ["BullExpansion", "BearExpansion", "Consolidation", "Manipulation", "CrisisStress"]
EVALUATED_OHLCV_ROOTS = ["BullExpansion", "BearExpansion", "Consolidation", "CrisisStress"]
...
def score_candidates_for_label(
    ...
    for threshold in thresholds:
        candidate = Candidate(
            ...
            f"v4_decile_log_odds_score_{label} >= {threshold:.12g}",
            scores >= threshold,
...
def schema(df: pd.DataFrame, numeric_cols: list[str], boolean_cols: list[str]) -> dict[str, Any]:
    return {
        "schema_id": "RootTaxonomyV4TargetGate",
        ...
        "target_definitions": {
            "BullExpansion": "source_backed_root_label == BullExpansion",
            "BearExpansion": "source_backed_root_label == BearExpansion",
            "Consolidation": "source_backed_root_label == Consolidation",
            "CrisisStress": "source_backed_root_label == CrisisStress",
            "Manipulation": "not materialized without direct tick/order-flow/L2/L3/order-lifecycle/event/social/on-chain evidence",
            "TransitionRecovery": "optional/conditional overlay; not required in this V4 release gate",
            "UnknownOrMixed": "residual abstention bucket; not release confidence",
        },
        "target_counts_by_split": {
            split: df[df["split"].eq(split)]["source_backed_root_label"].value_counts().to_dict()
...
def crosswalk() -> dict[str, Any]:
    return {
        "schema_id": "RootTaxonomyV4Crosswalk",
        "rule": "sub-regime packets remain context/guardrails until the V4 root label itself passes this gate",
        "crosswalk": {
            "BullExpansion": {
            ...
            "Consolidation": {
                ...
                "promotion_requirement": "range/compression target passes V4 root gate directly",
            },
            "Manipulation": {
...
def main() -> int:
    ...
    labels = df["source_backed_root_label"].astype(str).to_numpy()
    ...
    decision = {
        ...
        "accepted_gate": "accepted_95_all_RootTaxonomyV4_required_roots" if not missing else "partial_for_RootTaxonomyV4",
        "accepted_95_all_root_taxonomy_v4_roots": not missing,
        ...
        "next_action": (
            ...
            "stronger cross-provider signed expansion/consolidation inputs before rerunning unchanged RootTaxonomyV4 gates."
        ),
    }
    ...
    report = {
        "schema_version": "root-taxonomy-v4-model-gate/v1",
        "loop_id": LOOP_ID,
        "run_root": repo_rel(RUN_ROOT),
        "objective": "Materialize RootTaxonomyV4 target schema/crosswalk and rerun unchanged 95 gates with train-only generated rules and log-odds score candidates.",
    ...
    report_path = ROOT_DIR / "root_taxonomy_v4_model_gate_report.json"
    summary_path = ROOT_DIR / "root_taxonomy_v4_model_gate_summary.csv"
    schema_path = ROOT_DIR / "root_taxonomy_v4_target_schema.json"
    crosswalk_path = ROOT_DIR / "root_taxonomy_v4_crosswalk.json"
    ...
    (CHECKS_DIR / "root_taxonomy_v4_model_gate_assertions.out").write_text(
        "\n".join(
            [
    ...
    (RUN_ROOT / "README.md").write_text(
        "# RootTaxonomyV4 Model Gate\n\n"
        "Materializes the V4 target schema/crosswalk and evaluates train-only rule/log-odds candidates. "
