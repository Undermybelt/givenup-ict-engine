...
def build_qqq_same_market_lane(root: Path) -> tuple[LaneSpec, pd.DataFrame] | None:
    """Build the Board-A eligible same-market QQQ lane from long YF+IBKR artifacts."""
    provider_root = root.parent / "20260510T183454/provider"
    yf_path = provider_root / "yf_QQQ_1h_20240601_20260509.csv"
    ibkr_path = provider_root / "ibkr_QQQ_1h.csv"
    if not yf_path.exists() or not ibkr_path.exists():
        return None
    qqq_yf = _prefix_ohlcv(_read_qqq_yf_csv(yf_path), "qqq_yf_1h")
    qqq_ibkr = _read_qqq_yf_csv(ibkr_path)
    df = _asof_merge(qqq_yf, qqq_ibkr, "qqq_ibkr_1h", "75min")
    df = df.dropna(subset=["qqq_yf_1h_close", "qqq_ibkr_1h_close"]).reset_index(drop=True)
    return (
        LaneSpec(
            lane_id="qqq_yf_ibkr_same_market_provider_agreement_1h",
            market="QQQ",
            timeframe="1h",
            provider_observations=("qqq_yfinance_1h", "qqq_ibkr_1h"),
            board_eligible=True,
            board_eligibility_note="Same-market QQQ provider agreement from yfinance and IBKR 1h bars over the long refreshed overlap.",
        ),
        df,
    )


def build_qqq_ibkr_nq_cache_cross_lane(root: Path, aq_root: Path) -> tuple[LaneSpec, pd.DataFrame] | None:
    provider_root = root.parent / "20260510T183454/provider"
    yf_path = provider_root / "yf_QQQ_1h_20240601_20260509.csv"
    ibkr_path = provider_root / "ibkr_QQQ_1h.csv"
    if not yf_path.exists() or not ibkr_path.exists():
    ...
    nq_1d = _read_aq_feather(aq_root, "1d")
    df = _asof_merge(qqq_yf, qqq_ibkr, "qqq_ibkr_1h", "75min")
    df = _asof_merge(df, nq_1h, "nq_aq_1h", "75min")
    df = _asof_merge(df, nq_15m, "nq_aq_15m", "20min")
    df = _asof_merge(df, nq_4h, "nq_aq_4h", "5h")
    df = _asof_merge(df, nq_1d, "nq_aq_1d", "36h")
    df = df.dropna(
        subset=["qqq_yf_1h_close", "qqq_ibkr_1h_close", "nq_aq_1h_close", "nq_aq_15m_close"]
    ).reset_index(drop=True)
    return (
        LaneSpec(
            lane_id="qqq_yf_ibkr_nq_cache_cross_provider_agreement_1h",
            market="QQQ,NQ",
            timeframe="1h",
            ...
            board_eligible=True,
            board_eligibility_note="Cross-market QQQ/NQ lane: QQQ has same-market yfinance+IBKR provider breadth, NQ contributes local Auto-Quant multi-timeframe regime context.",
        ),
        df,
    )
...
def run_lane(lane: LaneSpec, raw: pd.DataFrame, out_dir: Path) -> dict[str, Any]:
    if lane.lane_id.startswith("qqq_yf_ibkr"):
        if "nq_cache_cross" in lane.lane_id:
            provider_prefixes = ["qqq_yf_1h", "qqq_ibkr_1h", "nq_aq_1h", "nq_aq_15m", "nq_aq_4h", "nq_aq_1d"]
            primary_close = "nq_aq_1h_close"
        else:
            provider_prefixes = ["qqq_yf_1h", "qqq_ibkr_1h"]
            primary_close = "qqq_yf_1h_close"
    elif lane.lane_id.startswith("qqq_nq"):
...
def main() -> None:
    ...
    same_market_lane = build_qqq_same_market_lane(root)
    if same_market_lane is not None:
        lanes.append(same_market_lane)
    qqq_nq_cross_lane = build_qqq_ibkr_nq_cache_cross_lane(root, aq_root)
    if qqq_nq_cross_lane is not None:
        lanes.append(qqq_nq_cross_lane)
    lanes.extend(
        [
    ...
    if args.only_lane:
        wanted = set(args.only_lane)
        lanes = [(lane, raw) for lane, raw in lanes if lane.lane_id in wanted]
        if not lanes:
            raise SystemExit(f"no lanes matched --only-lane values: {sorted(wanted)}")
