...
class TomacNQ_VM_Conservative48_AM13_14(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def populate_indicators_4h(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["ema_slow"] = ta.EMA(dataframe, timeperiod=144)
        return dataframe

    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["high_window"] = dataframe["high"].rolling(48).max().shift(1)
        dataframe["low_window"] = dataframe["low"].rolling(48).min().shift(1)
        dataframe["hour_utc"] = dataframe["date"].dt.hour
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        am_killzone = (dataframe["hour_utc"] >= 13) & (dataframe["hour_utc"] <= 14)
