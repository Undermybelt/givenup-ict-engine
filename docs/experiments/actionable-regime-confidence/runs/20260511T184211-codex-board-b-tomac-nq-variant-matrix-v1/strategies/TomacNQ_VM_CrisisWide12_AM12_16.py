...
class TomacNQ_VM_CrisisWide12_AM12_16(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["high_window"] = dataframe["high"].rolling(12).max().shift(1)
        dataframe["low_window"] = dataframe["low"].rolling(24).min().shift(1)
        dataframe["hour_utc"] = dataframe["date"].dt.hour
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        am_killzone = (dataframe["hour_utc"] >= 12) & (dataframe["hour_utc"] <= 16)
        breakout = dataframe["close"] > dataframe["high_window"]
        entry = am_killzone & breakout
