...
RUN_ID = "20260512T173141+0800-codex-ote-level-leaf-six-provider-aq-v1"
...
BRANCH_PATHS = [
    "TrendExpansion -> NormalVolatility -> OTERetracement050 -> ote_pullback_050_v1",
    "TrendExpansion -> NormalVolatility -> OTERetracement0618 -> ote_pullback_0618_v1",
    "TrendExpansion -> NormalVolatility -> OTERetracement0705 -> ote_pullback_0705_v1",
    "TrendExpansion -> NormalVolatility -> OTERetracement0786 -> ote_pullback_0786_v1",
]
...
def material(package_id: str, title: str, aq_symbol: str, data_path: Path, provider_note: str) -> dict[str, Any]:
    return {
        ...
        "strategy_source_path": str(RUN_ROOT / "agent-material/ProviderOteLevelPullbackV1.py"),
        "strategy_class_name": "ProviderOteLevelPullbackV1",
        "strategy_brief": "Board B OTE level-leaf pullback profitability factor with separate retracement leaves.",
        "evaluation_priority": [
...
def main() -> int:
    ...
    write_text(RUN_ROOT / "agent-material/ProviderOteLevelPullbackV1.py", STRATEGY_SOURCE)
    ...
    for label, args, raw_path, norm_path, provider, note, aq_symbol, display_symbol in fetch_jobs:
        ...
        if exit_code == 0 and raw_path.exists():
            ...
            package_id = f"ote-level-leaf-{provider.lower().replace('/', '-').replace(' ', '-')}-1h-v1"
            material_path = RUN_ROOT / "agent-material" / f"{package_id}.material.json"
            write_json(material_path, material(package_id, f"OTE level leaves - {provider} {display_symbol} 1h", aq_symbol, norm_path, note))
    ...
    tvr_exit = run_or_reuse(
        ...
        [
            ...
            "fetch",
            "--market",
            "board-b-173141-ote-tvr-btc-usd-1h",
            "--interval",
            "1h",
            "--role",
            "crypto_reference",
        ...
        env={
            "HOME": "/tmp/ict-engine-tvr-local-stdio-173141-home",
    ...
    if tvr_exit == 0:
        ...
        package_id = "ote-level-leaf-tvr-btc-usd-1h-v1"
        material_path = RUN_ROOT / "agent-material" / f"{package_id}.material.json"
        write_json(material_path, material(package_id, "OTE level leaves - TVR BTC-USD 1h", "BTC", tvr_norm, "source_provider=TradingViewRemix/TVR local-stdio BTC-USD 1h current fetch"))
    ...
    write_json(RUN_ROOT / "summaries/ote_level_leaf_provider_preflight_v1.json", summary)
