...
RUN_ID = "20260511T204803-codex-live-public-acquisition-recheck-v36"
...
KAGGLE_PULL_ROOT = Path("/tmp/ict-engine-kaggle-stock-regimes-live-refresh-v36")
...
def main() -> int:
    ...
    decision = (
        "live_public_acquisition_recheck_v36=public_recency_rows_materialized_needs_verifier_review"
        if new_rows_acquired
        else "live_public_acquisition_recheck_v36=no_new_public_rows_intakes_still_absent_blocked"
    )
    ...
    (OUT / "live_public_acquisition_recheck_v36.json").write_text(
        json.dumps(audit, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    write_csv(
        OUT / "live_public_acquisition_recheck_v36_intake_roots.csv",
        roots,
        ["id", "requirements", "root", "required_files", "present_files", "missing_files", "exists", "ready"],
    )
    write_csv(
        OUT / "live_public_acquisition_recheck_v36_kaggle_targets.csv",
    ...
    lines.extend(
        [
            ...
            "## Artifacts",
            "",
            f"- JSON: `{OUT / 'live_public_acquisition_recheck_v36.json'}`",
            f"- Intake-root CSV: `{OUT / 'live_public_acquisition_recheck_v36_intake_roots.csv'}`",
            f"- Kaggle target CSV: `{OUT / 'live_public_acquisition_recheck_v36_kaggle_targets.csv'}`",
            f"- Assertions: `{CHECKS / 'live_public_acquisition_recheck_v36_assertions.out'}`",
        ]
    )
    (OUT / "live_public_acquisition_recheck_v36.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    (CHECKS / "live_public_acquisition_recheck_v36_assertions.out").write_text(
        "\n".join(assertions) + "\n", encoding="utf-8"
    )
