#!/usr/bin/env python3
"""Refresh Board A coverage disposition after public crypto fetch matrices."""
...
RUN_ID = "20260511T073200+0800-codex-public-crypto-disposition-refresh"
RUN_ROOT = REPO / "docs/experiments/actionable-regime-confidence/runs/20260511T073200-codex-public-crypto-disposition-refresh"
...
BINANCE_BYBIT = REPO / "docs/experiments/actionable-regime-confidence/runs/20260511T073000-codex-binance-bybit-public-matrix/public-crypto-matrix/binance_bybit_public_matrix.json"
...
def main() -> None:
    ...
    binance_bybit = load_json(BINANCE_BYBIT)
    ...
    for cell in binance_bybit["rows"]:
        if cell["status"] == "ok":
            add_row(
                rows,
                cell["provider"],
                ...
                "bar_root",
                "unsupported_for_accepted_confidence",
                f"{cell['provider']}_data_available_but_missing_independent_source_backed_root_labels",
                f"ok_rows={cell['rows']}",
                "missing_independent_root_labels",
            )
        else:
            add_row(
                ...
                cell["reason"],
                cell["status"],
                "not_labeled_fetch_failed",
            )

    ready_not_attempted = {"ibkr": catalog["ibkr_symbols"]}
    ...
    report = {
        "run_id": RUN_ID,
        "goal_achieved": False,
        "objective": "Refresh full coverage disposition after Binance, Bybit, and Kraken public crypto matrices are fetched under the uv wrapper.",
        "source_artifacts": {
            ...
            "binance_bybit_public_matrix": rel(BINANCE_BYBIT),
        ...
        "completion_accounting": {
            ...
            "why_not_accepted": [
                "Yfinance, Binance, Bybit, and Kraken cells still lack independent source-backed MainRegimeV2 root labels.",
                "IBKR and Polymarket are ready under uv wrapper but not data-attempted in this slice.",
        ...
        "thresholds_relaxed": False,
        "trade_usable": False,
        "gate_result": "public_crypto_disposition_refreshed_remaining_cells_pending",
        "next_action": "Attempt the IBKR ready lane with a low-pollution uv wrapper, or mark each IBKR symbol/timeframe cell with its concrete operator-runtime fetch result before any completion audit.",
        "artifacts": {
            "disposition_json": rel(OUT_DIR / "public_crypto_disposition_refresh.json"),
            "disposition_md": rel(OUT_DIR / "public_crypto_disposition_refresh.md"),
            "disposition_csv": rel(OUT_DIR / "public_crypto_disposition_refresh.csv"),
            "assertions": rel(CHECK_DIR / "public_crypto_disposition_refresh_assertions.out"),
            "script": rel(Path(__file__)),
        },
    }

    (OUT_DIR / "public_crypto_disposition_refresh.json").write_text(
        json.dumps(report, indent=2, sort_keys=True) + "\n"
    )
    with (OUT_DIR / "public_crypto_disposition_refresh.csv").open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    lines = [
        "# Public Crypto Disposition Refresh",
        "",
        f"Run id: `{RUN_ID}`",
        "",
        "Goal achieved: `false`",
    ...
    lines.extend(
        [
            "",
            "Gate result: `public_crypto_disposition_refreshed_remaining_cells_pending`",
            "",
            "Runtime code changed: false. Thresholds relaxed: false. Trade usable: false.",
        ]
    )
    (OUT_DIR / "public_crypto_disposition_refresh.md").write_text("\n".join(lines) + "\n")
    ...
    (CHECK_DIR / "public_crypto_disposition_refresh_assertions.out").write_text(
        "\n".join(assertion_lines) + "\n"
    )
    print(rel(OUT_DIR / "public_crypto_disposition_refresh.json"))
