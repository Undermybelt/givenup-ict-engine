...
RUN_ID = "20260512T184139+0800-codex-vwap-session-liquidity-six-provider-aq-v1"
...
def material(package_id: str, title: str, symbol: str, data_path: Path, provider_note: str) -> dict[str, Any]:
    return {
        ...
        "strategy_source_path": str(RUN_ROOT / "agent-material/ProviderVwapSessionLiquidityV1.py"),
        "strategy_class_name": "ProviderVwapSessionLiquidityV1",
        "strategy_brief": "Board B VWAP/session/liquidity branch packet.",
        "evaluation_priority": [
        ...
        "consumer_evidence_profile": {
            "main_regime": "RangeConsolidation_or_TrendExpansion_or_VolatilityCompression_or_Transition",
...
def main() -> int:
    ...
    write_text(RUN_ROOT / "agent-material/ProviderVwapSessionLiquidityV1.py", STRATEGY_TEXT)
    ...
    for label, args, raw_path, norm_path, provider, note, aq_symbol, provider_symbol in fetch_jobs:
        ...
        if exit_code == 0 and raw_path.exists():
            ...
            package_id = f"vwap-session-liquidity-{provider.lower().replace('/', '-').replace(' ', '-')}-1h-v1"
            material_path = RUN_ROOT / "agent-material" / f"{package_id}.material.json"
            write_json(material_path, material(package_id, f"VWAP session liquidity - {provider} {provider_symbol} 1h", aq_symbol, norm_path, note))
    ...
    tvr_exit = run_or_reuse(
        ...
        [
            ...
            "fetch",
            "--market",
            "board-b-184139-vwap-session-liquidity-tvr-btc-usd-1h",
            "--interval",
            "1h",
            "--role",
            "crypto_reference",
        ...
        env={
            "HOME": "/tmp/ict-engine-tvr-local-stdio-184139-home",
    ...
    if tvr_exit == 0:
        ...
        package_id = "vwap-session-liquidity-tvr-btc-usd-1h-v1"
        material_path = RUN_ROOT / "agent-material" / f"{package_id}.material.json"
        write_json(material_path, material(package_id, "VWAP session liquidity - TVR BTC-USD 1h", "BTC", tvr_norm, "source_provider=TradingViewRemix/TVR local-stdio BTC-USD 1h current fetch"))
    ...
    write_json(RUN_ROOT / "summaries/vwap_session_liquidity_provider_preflight_v1.json", summary)
