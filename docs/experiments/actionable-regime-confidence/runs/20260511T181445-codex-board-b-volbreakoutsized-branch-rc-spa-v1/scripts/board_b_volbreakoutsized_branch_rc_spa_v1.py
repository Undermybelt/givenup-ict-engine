#!/usr/bin/env python3
"""Run VolBreakoutSized through Board B branch-path RC-SPA scoring."""
...
BASE_SCRIPT = (
    ...
    "scripts/board_b_crashrebound_branch_rc_spa_v1.py"
)

spec = importlib.util.spec_from_file_location("branch_rc_spa_base", BASE_SCRIPT)
...
RUN_ID = "20260511T181445+0800-codex-board-b-volbreakoutsized-branch-rc-spa-v1"
STRATEGY_NAME = "VolBreakoutSized"
RUN_ROOT = Path(
    "docs/experiments/actionable-regime-confidence/runs/"
...
TRADES_CSV = OUT_DIR / "volbreakoutsized_branch_trades_v1.csv"
TRADES_JSONL = OUT_DIR / "volbreakoutsized_branch_trades_v1.jsonl"
LABELS_JSONL = OUT_DIR / "volbreakoutsized_purged_cv_labels_v1.jsonl"
SUMMARY_JSON = OUT_DIR / "volbreakoutsized_branch_summary_v1.json"
RC_SPA_JSON = OUT_DIR / "volbreakoutsized_rc_spa_report_v1.json"
PAYOFF_JSON = OUT_DIR / "volbreakoutsized_payoff_shape_v1.json"
PURGED_JSON = OUT_DIR / "volbreakoutsized_purged_cv_guard_v1.json"
PACKET_JSON = OUT_DIR / "volbreakoutsized_regime_profitability_packet_v1.json"
IMPORT_MANIFEST = OUT_DIR / "volbreakoutsized_strategy_library_for_import_v1.json"
REPORT_MD = OUT_DIR / "volbreakoutsized_branch_rc_spa_v1.md"
ASSERTIONS = CHECK_DIR / "board_b_volbreakoutsized_branch_rc_spa_v1_assertions.out"

AUTO_QUANT_ROOT = Path("/Users/thrill3r/Auto-Quant")
AUTO_QUANT_STRATEGY_PATH = AUTO_QUANT_ROOT / "versions/0.4.0/strategies"
STRATEGY_FILE = AUTO_QUANT_STRATEGY_PATH / f"{STRATEGY_NAME}.py"
PAIR_BASKET = ["BTC/USDT", "ETH/USDT", "SOL/USDT", "BNB/USDT", "AVAX/USDT"]

ROOT_TO_SUB_REGIME = {
    "Bull": "TrendExpansion",
    "Bear": "BearReliefBreakout",
    "Sideways": "RangeBreakoutAttempt",
    "Crisis": "CrisisBreakoutSuppression",
}
SUB_SUB_FACTOR = "Donchian24Ema4hTrendVolumeVolTarget"
PROFIT_FAMILY = "TrendBreakoutVolTarget"
...
def install_constants() -> None:
    replacements = {
        ...
        "AUTO_QUANT_STRATEGY_PATH": AUTO_QUANT_STRATEGY_PATH,
...
def run_backtest(label: str, timerange: str) -> dict:
    args = {
    ...
    config = base.Configuration(args, base.RunMode.BACKTEST).get_config()
    ...
    with log_path.open("w", encoding="utf-8") as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        bt = base.Backtesting(config)
        bt.start()
    strat = bt.results.get("strategy", {}).get(STRATEGY_NAME, {}) or {}
    aggregate, per_pair = base._extract_metrics(bt.results, STRATEGY_NAME)
    return {
        "label": label,
        "timerange": timerange,
        ...
        "strategies": [
            {
                ...
                "metadata": {
                    "strategy": STRATEGY_NAME,
                    "mutation_id": "board-b-volbreakoutsized-branch-rc-spa-v1",
                    "base_factor": "auto_quant_v0_4_0_volbreakoutsized",
                    "hypothesis": "Per-pair Donchian breakout with 4h trend regime gate, volume confirmation, and ATR/close vol-target sizing.",
                    "paradigm": "trend breakout with volatility-targeted sizing",
                    "expected_regime": "rooted_market_regime_context_then_trend_expansion_branch",
                    "factors_used": [
                    ...
                    "parent": "versions/0.4.0/VolBreakoutSized",
...
def main() -> int:
    install_constants()
    base.run_backtest = run_backtest
    for path in [OUT_DIR, CHECK_DIR, LOG_DIR]:
        path.mkdir(parents=True, exist_ok=True)
    if not STRATEGY_FILE.exists():
        raise FileNotFoundError(STRATEGY_FILE)
    if not base.SOURCE_REGIME_CSV.exists():
        raise FileNotFoundError(base.SOURCE_REGIME_CSV)

    timerange_results = [run_backtest(label, timerange) for label, timerange in base.TIMERANGES]
    ...
    summary = {
        "schema_version": "board-b-volbreakoutsized-branch-summary/v1",
        "run_id": RUN_ID,
    ...
    base._write_json(PACKET_JSON, branch_packet)
    base._write_json(IMPORT_MANIFEST, import_manifest)
