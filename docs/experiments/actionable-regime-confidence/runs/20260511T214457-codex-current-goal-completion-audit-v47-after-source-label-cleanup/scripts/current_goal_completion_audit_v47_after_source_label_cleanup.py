#!/usr/bin/env python3
"""Post-source-label-cleanup completion audit for Board A.
...
RUN_ID = "20260511T214457-codex-current-goal-completion-audit-v47-after-source-label-cleanup"
...
ARTIFACTS = {
    ...
    / "20260511T212711-codex-current-goal-completion-audit-v44-post-212325/"
    "completion-audit/current_goal_completion_audit_v44_post_212325.json",
...
def main() -> int:
    ...
    checklist = [
        ...
        },
        {
            ...
            "status": "fail_blocked",
            "gap": "Strict all-regime 95% objective is still not achieved; source-label equivalence is schema-ready but unscored and partial, R3/R5 roots remain absent, and R6 direct confidence is below 0.95.",
        },
        {
            "id": "R2_other_market_validation",
            "requirement": "Validate regimes on other markets/species with suitable confidence.",
            "evidence": f"source_label_equivalence_status={source_label_status}; rows={source_label_rows}; root=/tmp/ict-engine-source-label-equivalence-intake",
            "status": "fail_blocked",
            "gap": "Source-label equivalence rows are schema-ready/unscored with all four labels, but still not a calibrated >=95 confidence acceptance package.",
        },
        {
    ...
    decision = f"current_goal_completion_audit_v47=after_source_label_cleanup_{len(ready_roots)}of4_roots_still_blocked"
    result = {
        "run_id": RUN_ID,
        ...
        "trade_usable": False,
        "next_action": "Complete the source-label equivalence root with Bear and confidence scoring, acquire native sub-hour and recency-extension roots, and expand R6 with 50/50 broad normal controls/direct species coverage before any completion claim.",
    }

    json_path = OUT_DIR / "current_goal_completion_audit_v47_after_source_label_cleanup.json"
    checklist_csv = OUT_DIR / "current_goal_completion_audit_v47_checklist.csv"
    gates_csv = OUT_DIR / "current_goal_completion_audit_v47_gates.csv"
    roots_csv = OUT_DIR / "current_goal_completion_audit_v47_intake_roots.csv"
    report_path = OUT_DIR / "current_goal_completion_audit_v47_after_source_label_cleanup.md"
    assertions_path = CHECK_DIR / "current_goal_completion_audit_v47_after_source_label_cleanup_assertions.out"
    ...
    report_lines = [
        ...
        f"- Source-label equivalence verifier: `{source_label_status}`; rows `{source_label_rows}`; accepted confidence gate `false`.",
