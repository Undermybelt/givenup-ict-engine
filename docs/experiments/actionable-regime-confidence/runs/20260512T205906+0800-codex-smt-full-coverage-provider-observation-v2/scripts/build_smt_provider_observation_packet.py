...
def fail_closed_observation_row(spec: PairSpec, summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "correlation_window_bars": max(0, int(summary.get("aligned_rows", 0)) - 1),
        "recent_correlation": round(float(summary.get("correlation", 0.0)), 4),
        "event_time": None,
        "smt_signal": "none",
        "base_swing_type": "n/a",
        ...
        "evidence_source": "provider_backed_same_timeframe_aligned_candles_fail_closed",
        "confidence": 0.0,
        "fail_closed_reason": summary.get("fail_closed_reason") or "no_same_event_smt_confirmation_failure_detected",
...
def main() -> int:
    ...
    for spec in PAIR_SPECS:
        ...
        all_rows.extend(pair_rows if pair_rows else [fail_closed_observation_row(spec, summary)])
    ...
    for regime in ["trend", "range", "transition", "stress", "other"]:
        event_count = sum(1 for row in all_rows if row["regime_bucket"] == regime and row["smt_signal"] != "none")
        stats[regime] = {
            "win_rate": None,
            "trade_count": 0,
            ...
            "instrument_coverage": sorted(
                {row["base_symbol"] for row in all_rows if row["regime_bucket"] == regime and row["smt_signal"] != "none"}
            ),
            "confidence": 0.0,
