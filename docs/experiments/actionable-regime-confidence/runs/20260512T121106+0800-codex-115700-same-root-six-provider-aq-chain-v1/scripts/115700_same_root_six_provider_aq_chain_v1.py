...
RUN_ID = "20260512T121106+0800-codex-115700-same-root-six-provider-aq-chain-v1"
...
SOURCE_JSON = SOURCE_ROOT / "same-root-six-provider-1h-aq-v1" / "same_root_six_provider_1h_aq_v1.json"

OUT_DIR = ROOT / "command-output"
CHECK_DIR = ROOT / "checks"
REPORT_DIR = ROOT / "115700-same-root-six-provider-aq-chain-v1"
...
def provider_matrix_readback(source: dict[str, Any]) -> dict[str, Any]:
    ...
    fetch_rows = {
        ...
        "tvr_default_binance_btcusdt_1h": csv_rows(SOURCE_ROOT / "provider-csv" / "tvr_default_binance_btcusdt_1h.csv"),
        "ibkr_btc_paxos_30d_1h_midpoint": csv_rows(SOURCE_ROOT / "provider-csv" / "BTC_1h_midpoint.csv"),
    }
    fetch_exits = {
        ...
        "tvr_default_binance_btcusdt_1h": exit_code(SOURCE_ROOT / "checks" / "21_tvr_default_binance_btcusdt_1h.exit"),
        "ibkr_btc_paxos_30d_1h_midpoint": exit_code(SOURCE_ROOT / "checks" / "30_ibkr_paxos_btc_30d_1h_midpoint.exit"),
    }
    return {
        ...
        "same_root_six_provider_aq_authority": same_root_ready and all(value > 0 for value in fetch_rows.values()),
        "ibkr_midpoint_aq_success": fetch_rows["ibkr_btc_paxos_30d_1h_midpoint"] > 0,
        "authority_note": "all six provider CSVs and AQ workspaces are read from the single 115700 root at comparable 1h granularity",
    }
    ...
    for result in results:
        ...
        for path in sorted((workspace / "derived").glob("*.real_trades.jsonl")):
            ...
            for line in path.read_text().splitlines():
                ...
                row["failure_reason"] = "realized_loss" if row.get("realized_outcome") == "loss" else "not_failure_realized_win"
...
def run_chain(trades_path: str, library_path: Path, provider_json: dict[str, Any]) -> dict[str, int]:
    ...
    commands = {
        ...
        "22_ingest_real_trades": ["./target/debug/ict-engine", "auto-quant-ingest-real-trades", "--symbol", SYMBOL, "--state-dir", str(STATE_DIR), "--trades", trades_path, "--source", "same_root_six_provider_aq_115700", "--force"],
...
def write_report(summary: dict[str, Any]) -> None:
    ...
    report = REPORT_DIR / "115700_same_root_six_provider_aq_chain_v1.md"
    assertions = CHECK_DIR / "115700_same_root_six_provider_aq_chain_v1_assertions.out"
    checklist = REPORT_DIR / "prompt_to_artifact_checklist_115700_same_root_six_provider_aq_chain_v1.csv"
    ...
    lines.extend(
        [
            ...
            f"- CatBoost feature-support exits: `{summary['support_exits']}`.",
            ...
            "",
            "## Artifacts",
            f"- JSON: `{REPORT_DIR / '115700_same_root_six_provider_aq_chain_v1.json'}`",
    ...
    with checklist.open("w", newline="") as fh:
        ...
        writer.writerow(["CatBoost feature support", str(PATH_RANKER_DIR / "catboost_feature_support_v1"), "covered", "augmented train/apply/register/enable attempted"])
    ...
    for label, code in summary["support_exits"].items():
        assertion_lines.append(("PASS" if code == 0 else "FAIL_CLOSED") + f" {label}_exit={code}")
...
def main() -> int:
    ...
    support_exits = run_augmented_catboost_support()
    cleanup_status = cleanup_catboost_info()
    summary = summarize(source, matrix, trade_summary, library_path, provider_json, chain_exits, support_exits, cleanup_status)
    write_json(REPORT_DIR / "115700_same_root_six_provider_aq_chain_v1.json", summary)
