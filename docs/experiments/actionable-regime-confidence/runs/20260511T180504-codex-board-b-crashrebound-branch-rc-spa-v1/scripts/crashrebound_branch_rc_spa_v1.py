#!/usr/bin/env python3
"""Board B CrashRebound branch-path RC-SPA evidence builder.
...
RUN_ID = "20260511T180504+0800-codex-board-b-crashrebound-branch-rc-spa-v1"
...
BRANCH_MAP = {
    "Bull": {
        "sub_regime_tags": "BullPullback",
        "sub_sub_regime_or_profit_factor": "OversoldDrawdownReboundSetup",
    ...
    "Bear": {
        ...
        "sub_sub_regime_or_profit_factor": "CapitulationBounceSetup",
        "profit_factor_family": "counter_trend_drawdown_rebound",
        "profit_factor_leaf": RECIPE_ID,
        "allowed_action": "research_only_until_rc_spa_passes",
    ...
    "Sideways": {
        "sub_regime_tags": "RangeStress",
        "sub_sub_regime_or_profit_factor": "MeanReversionReboundSetup",
        "profit_factor_family": "counter_trend_drawdown_rebound",
        "profit_factor_leaf": RECIPE_ID,
        "allowed_action": "research_only_until_rc_spa_passes",
    ...
    "Crisis": {
        ...
        "sub_sub_regime_or_profit_factor": "LongReboundSuppressionReview",
        "profit_factor_family": "counter_trend_drawdown_rebound",
        "profit_factor_leaf": RECIPE_ID,
...
def main() -> int:
    ...
    backtest_log = CHECK_DIR / "crashrebound_freqtrade_backtests.out"
    backtest_err = CHECK_DIR / "crashrebound_freqtrade_backtests.err"
    timerange_summaries = []
    ...
    write_csv(OUT_DIR / "crashrebound_branch_path_trades_v1.csv", branch_rows)
    write_csv(OUT_DIR / "crashrebound_timerange_backtest_summaries_v1.csv", timerange_summaries)
    ...
    write_csv(OUT_DIR / "crashrebound_branch_rc_spa_summary_v1.csv", branch_summaries)
    ...
    decision = {
        ...
        "primary_blocker": (
            "CrashRebound has real trade rows and root-first branch paths, but no branch path "
            "passes RC-SPA because branch-level trade depth/fold depth and single-recipe PBO "
            "hard gates fail; downstream runtime promotion remains blocked."
        ),
        "next_action": (
            "B3-repeat: source more branch-path trade rows from provider-backed Auto-Quant runs "
            "or choose a denser recipe before attempting Pre-Bayes -> BBN -> CatBoost -> "
            "execution-tree branch consumption."
        ),
    }
    ...
    provider_snapshot = {
        ...
        "provider_use_note": (
            "This loop consumed the existing CrashRebound Auto-Quant cache. Provider snapshots "
    ...
    report = {
        ...
        "artifacts": {
            "trade_rows_csv": str(
                (OUT_DIR / "crashrebound_branch_path_trades_v1.csv").relative_to(REPO_ROOT)
            ),
            "branch_summary_csv": str(
                (OUT_DIR / "crashrebound_branch_rc_spa_summary_v1.csv").relative_to(REPO_ROOT)
            ),
            "timerange_summary_csv": str(
                (OUT_DIR / "crashrebound_timerange_backtest_summaries_v1.csv").relative_to(
                    REPO_ROOT
                )
            ),
    ...
    report_path = OUT_DIR / "crashrebound_branch_rc_spa_report_v1.json"
    report_path.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    md_path = OUT_DIR / "crashrebound_branch_rc_spa_report_v1.md"
    lines = [
    ...
    lines.extend(
        [
            ...
            f"- Trade rows: `{(OUT_DIR / 'crashrebound_branch_path_trades_v1.csv').relative_to(REPO_ROOT)}`",
            f"- Branch summary: `{(OUT_DIR / 'crashrebound_branch_rc_spa_summary_v1.csv').relative_to(REPO_ROOT)}`",
            f"- Timerange summary: `{(OUT_DIR / 'crashrebound_timerange_backtest_summaries_v1.csv').relative_to(REPO_ROOT)}`",
    ...
    if total_trades != 207:
        assertions.append("ASSERT_FAIL:expected_207_full_5y_trades")
    if len(branch_summaries) < len(REQUIRED_BRANCH_PATHS):
        assertions.append("ASSERT_FAIL:missing_required_branch_summaries")
    (CHECK_DIR / "crashrebound_branch_rc_spa_v1_assertions.out").write_text(
        "\n".join(assertions) + "\n",
        encoding="utf-8",
    )
