...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| NQ 1m | 3.12% | 62.50% | 59.62% | 76.33% | 71.88% | 100.00% |
| NQ 5m | 0.00% | 70.00% | 57.99% | 75.53% | 70.00% | 100.00% |
| NQ mtf | 11.76% | 58.82% | 59.28% | 76.34% | 64.71% | 100.00% |
| NQ htf | 20.00% | 60.00% | 62.24% | 77.17% | 80.00% | 100.00% |
...
- 重新复跑后，原有 NQ m1 / m5 / mtf / htf 行的样本数与指标保持不变
- 修正后，`state/YM` / `state/GC` / `state/CL` 的 1m 验证样本数统一为 34

**Multi-market opt-in probe（2026-05-08）**：
| 数据 | 默认高置信 | Opt-in 高置信 | 默认平均置信 | Opt-in 平均置信 | 默认可交易 | Opt-in 可交易 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| YM 1m | 0.00% | 52.94% | 57.75% | 75.65% | 55.88% | 100.00% |
| GC 1m | 2.94% | 47.06% | 57.49% | 75.16% | 67.65% | 100.00% |
| CL 1m | 2.94% | 41.18% | 55.01% | 73.98% | 44.12% | 100.00% |

**Multi-market htf probe（2026-05-08）**：
| 数据 | 默认高置信 | Opt-in 高置信 | 默认平均置信 | Opt-in 平均置信 | 默认可交易 | Opt-in 可交易 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| YM htf | 10.00% | 60.00% | 59.64% | 75.88% | 70.00% | 100.00% |
| GC htf | 40.00% | 80.00% | 69.95% | 83.01% | 80.00% | 100.00% |
| CL htf | 10.00% | 40.00% | 57.75% | 75.23% | 50.00% | 100.00% |

**Multi-market mtf probe（2026-05-08）**：
| 数据 | 默认高置信 | Opt-in 高置信 | 默认平均置信 | Opt-in 平均置信 | 默认可交易 | Opt-in 可交易 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| YM mtf | 5.88% | 64.71% | 60.17% | 77.03% | 70.59% | 100.00% |
| GC mtf | 5.88% | 52.94% | 56.65% | 74.39% | 52.94% | 100.00% |
| CL mtf | 11.76% | 64.71% | 62.81% | 77.83% | 82.35% | 100.00% |

**Multi-market opt-in 结论**：
- NQ / YM / GC / CL 的 opt-in profile 都能把高置信比例抬到 30% 以上
...
- 真实 NQ / YM / GC / CL 本地数据在默认路径下平均置信度大多落在 55-60%，但 GC htf 已经自然达到更高置信区间，说明默认路径并非一刀切。
- opt-in profile 证明了高置信比例可以跨 NQ / YM / GC / CL，在 1m / mtf / htf 样本上稳定抬升到 40%+。
- 下一个真正的质量门槛是扩展更多市场类别和多周期样本，继续用 `--profile` / `--config` 做可复现调参。

---

**更新时间**：2026-05-08 13:27
**更新人**：Codex
