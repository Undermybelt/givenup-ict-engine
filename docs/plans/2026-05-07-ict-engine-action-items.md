...
| v2 | 53.05% | 0% | 41.67% | 降低趋势阈值 + 流动性阈值 |
| v3 | 56.10% | 0% | 62.50% | 流动性基础置信度 0.3 |
| v4 | 54.36% | 0% | 58.33% | 结构权重 45% + 一致性 30% |
| v5 | **60.06%** | **4.17%** | **75.00%** | 基础置信度 0.15 + 一致性部分得分 |
...
- ExtremeStress: 41.7%
- TrendExpansion: 33.3%
- RangeConsolidation: 25%

**关键调优点**：
1. 基础置信度 0.15 避免过低综合置信度
2. 结构权重 50%（趋势识别核心）
3. 一致性部分匹配得分（避免全有全无）
4. 流动性/波动率基础置信度提升
...
**更新时间**：2026-05-08 09:45
**更新人**：Claude (Hermes Agent)
**状态**：置信度调优完成阶段性目标，等待真实数据验证
