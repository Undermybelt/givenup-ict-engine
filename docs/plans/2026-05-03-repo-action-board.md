...
- [x] That `policy-training-status --human` / `--compact` low-token contract is now backed by real CLI verification against an empty `/tmp` state dir, not just by shell-layer implementation intent.
- [x] The policy-training low-token shell path now centralizes its three-line render contract in one helper instead of repeating `println!` calls inline, reducing the chance of future surface drift between the human and compact aliases.
