...
- [~] That canonical workflow provenance is still asymmetric: the current ensemble surface can derive `policy_runtime_line`, but persisted `EnsembleVoteRecord` and the ensemble-history path still do not carry the same signal as first-class data, so the full consumer closure remains open.
