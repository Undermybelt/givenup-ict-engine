...
|---|---|---|
| Treat `docs/plans/20260501repo.md` as implementation input | P6 path-ranking rows and the later source-reliability, duration, and off-policy diagnostics all map to the document's CatBoost, Dawid-Skene, HSMM, and OPE sections | partial |
| Land decisions as versioned repo artifacts | Commits `acce819`, `2bb1c8e`, `bb57d73`, `2253bfe`, `05caf1d`, `45fc44c`, `05d4ca7`, `d9631fc`, `400b00c`, `983e622`, `3f67add`, and `1cc6825` are committed on `green-baseline` | done for current slices |
| Preserve zero-config behavior | New path-ranking, source-reliability, duration, and SNIPS/DR diagnostics are derived from existing structural state/export rows; no new required CLI flags or environment variables were added | done for current slices |
...
- `live feedback posterior update` now has SNIPS effective-sample diagnostics, but still lacks deeper calibrated target-policy estimation.
...
- `structural_prior_state` now has empirical HSMM-style duration distributions and compact temporal-summary diagnostics, but still lacks BOCPD-style break detection.
...
3. Move to a non-P6 TODO slice: BOCPD-style break detection, deeper target-policy calibration, or explicit source-reliability EM.
