...
- P6 now has target-policy diagnostic feature columns, explicit opt-in external artifact registration/clearing and score-application paths, cumulative history-backed export datasets, and an external path-ranker artifact status boundary, but still lacks a real trained external path-ranker artifact/service.
