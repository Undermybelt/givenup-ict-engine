...
- P6 now has target-policy diagnostic feature columns, explicit opt-in external artifact registration/clearing and score-application paths, cumulative history-backed export datasets, history-backed readiness counters, and an external path-ranker artifact status boundary, but still lacks a real trained external path-ranker artifact/service.
...
- `BBN node/branch posterior update` now has maintained branch and node transition posterior state plus bounded discounted recursive node fallback, but still lacks deeper multi-step Hamilton/DBN recursive filtering beyond those compact transition surfaces.
