...
| Hamilton / DBN recursive filtering from plan | Branch transition posterior state and node transition posterior state persist; downstream node/regime surfaces now use direct node transition posteriors plus discounted two-step node fallback before branch/candidate fallback | `cargo test --lib transition_adjusted_node_posteriors`; `cargo check --all-targets` | partial: no deeper multi-step Hamilton/DBN recursive filter |
...
- `BBN node/branch posterior update` now has maintained branch and node transition posterior state plus discounted two-step node fallback, but still lacks deeper multi-step Hamilton/DBN recursive filtering beyond those compact transition surfaces.
