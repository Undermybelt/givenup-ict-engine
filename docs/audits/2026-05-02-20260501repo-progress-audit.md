...
- P6 now has target-policy diagnostic feature columns, an explicit opt-in external artifact registration path, and an external path-ranker artifact status boundary, but still lacks a real trained external path-ranker artifact/service.
