...
|---|---|---|
| Treat `docs/plans/20260501repo.md` as implementation input | P6 path-ranking rows and the later source-reliability, Dawid-Skene readiness, duration, BOCPD calibration, off-policy, target-policy variance, and delayed-reward censoring diagnostics all map to the document's CatBoost, Dawid-Skene, HSMM, BOCPD, delayed-feedback, and OPE sections | partial |
| Land decisions as versioned repo artifacts | Commits `acce819`, `2bb1c8e`, `bb57d73`, `2253bfe`, `05caf1d`, `45fc44c`, `05d4ca7`, `d9631fc`, `400b00c`, `983e622`, `3f67add`, `1cc6825`, `c494b60`, `0cbd46e`, `1cbf9fc`, `4cc3d66`, `0175b3c`, `fc04494`, `a880cb4`, `3fb108f`, `de0f1bb`, `eb3cff2`, `aee6e83`, `ffae6fd`, `f5714d3`, `7c1dcf6`, `df0e14f`, `2a8e25f`, and `afbc93b` are committed on `green-baseline` | done for current slices |
...
| Implement scoped slices and commit as appropriate | Recent code commits cover path-ranking target rows, source reliability/confusion, duration distributions, SNIPS/DR/target-policy diagnostics, target-policy probability diagnostics, EM readiness, fixed-iteration EM fit diagnostics, persisted EM source-confusion summaries, EM calibration diagnostics, EM reliability consumption, BOCPD calibration, compact run-length diagnostics, and delayed-reward censoring-adjusted diagnostics | `git log --oneline` on `green-baseline` | done for current slices |
...
| HSMM / BOCPD duration prior from plan | Empirical dwell distribution, hazard/survival, evidence-weighted BOCPD raw/calibrated break probability, compact empirical run-length mode/probability/tail/mass diagnostics, and temporal summary fields exist | `cargo test --lib duration`; `cargo test --lib workflow_status_phase_structural_temporal_summary_exposes_discounted_masses`; `cargo test --lib structural_temporal_summary_node_prefers_persisted_temporal_state_streak_count` | partial: no full recursive run-length posterior model |
| Logged-bandit / OPE target-policy learning from plan | Behavior probability logging, logged probability variance/confidence/lower-bound diagnostics, IPS/SNIPS/DR, ESS, target-policy reward prior, variance penalty, conservative lower bound, compact maturity/censoring counters, smoothed delayed-reward resolution/censoring probabilities, and censoring-adjusted reward prior/lower-bound diagnostics exist | `cargo test --lib test_structural_feedback_records_snips_and_dr_policy_priors`; `cargo test --lib structural_experience_prior`; `cargo test --lib structural_prior_maturity_diagnostics_count_unresolved_followed_feedback`; `cargo test --lib workflow_status_phase_structural_experience_priors_tracks_current_lineage` | partial: no full target-policy probability model or survival/competing-risk delayed-reward model |
...
- `live feedback posterior update` now has logged behavior-policy probability variance/confidence/lower-bound diagnostics, ESS-weighted target-policy reward prior, variance penalty, conservative lower-bound diagnostics, compact maturity/censoring counters, smoothed delayed-reward resolution/censoring probabilities, and censoring-adjusted reward prior/lower-bound diagnostics, but still lacks a fully calibrated target-policy probability model and survival/competing-risk delayed-reward model.
...
- `structural_prior_state` now has empirical HSMM-style duration distributions plus compact evidence-weighted BOCPD-style raw/calibrated break/continue and run-length telemetry, but still lacks richer recursive run-length posterior change-point modeling.
...
2. Move to a non-P6 TODO slice: fuller recursive BOCPD run-length posterior modeling, full target-policy probability calibration, survival/competing-risk delayed-reward modeling, or real larger-panel source-reliability EM calibration validation once enough cross-source labels exist.
