...
| Hamilton / DBN recursive filtering from plan | Branch transition posterior state and node transition posterior state persist; downstream node/regime and branch/candidate surfaces consume maintained normalized transition posteriors before fallback | transition and structural workflow tests listed below | partial: no deeper multi-step Hamilton/DBN recursive filter |
...
- P6 now has target-policy diagnostic feature columns, explicit opt-in external artifact registration/clearing paths, and an external path-ranker artifact status boundary, but still lacks a real trained external path-ranker artifact/service.
- P6 now distinguishes raw-scored mature-row sufficiency from propensity-weighted production-validation sufficiency, but still lacks real exported raw-scored rows and calibrated propensity-covered rows sufficient for production validation.
...
- `BBN node/branch posterior update` now has maintained branch and node transition posterior state, but still lacks deeper multi-step Hamilton/DBN recursive filtering beyond compact transition posterior surfaces.
