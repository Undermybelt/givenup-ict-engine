...
| `P2` Live feedback posterior update | `部分实现` | explicit success/failure mass exists, but delayed / partial-compliance math is not complete |
...
Literature mechanisms still worth importing
- explicit fractional pseudo-count notation for every outcome class
- separate semantics for:
  - followed profitable
  - followed invalidated
  - not-followed
  - abandoned
  - delayed unresolved
...
Current repo gap
- feedback learning is real and no longer heuristic-only, but delayed / censored outcome math is still incomplete
- not-followed recommendations still do not have a fully principled off-policy update path
