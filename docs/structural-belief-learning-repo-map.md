...
- structural prior stats/source summaries persist behavior-policy probability, probability variance, probability confidence/lower-bound diagnostics, SNIPS reward prior, SNIPS effective sample size, doubly robust reward prior, target-policy calibration weight, variance penalty, calibrated reward prior, and conservative reward lower bound; `structural-experience-priors` exposes the compact correction fields
